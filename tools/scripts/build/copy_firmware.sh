...
if [ "$BOARD_CONFIG" == "mt2523_hdk" ]; then
   ...
   if [ "$MTK_GNSS_ENABLE" == 'y' ]; then
        ...
        if [ -f "$SOURCE_DIR/prebuilt/driver/chip/mt5932/wifi_image/mt5932_image_sdio.bin" ]; then
            cp $SOURCE_DIR/tools/config/$BOARD_CONFIG/download/wifi/flash_download.cfg $OUTPUT/
            cp $SOURCE_DIR/prebuilt/driver/chip/mt5932/wifi_image/mt5932_image_sdio.bin $OUTPUT/wifi_firmware.bin
         else
           cp $SOURCE_DIR/tools/config/$BOARD_CONFIG/download/wifi/flash_download.cfg $OUTPUT/
        fi
    else
...
elif [ "$BOARD_CONFIG" == "mt7697_hdk" ] || [ "$BOARD_CONFIG" == "mt76x7_hdk" ]; then
    ...
    if [ -f "$OUTPUT/iot_sdk_demo.bin" ] && [ "$BOARD_CONFIG" == "mt5932_hdk" ]; then
       echo "copy $OUTPUT/iot_sdk_demo.bin to $SOURCE_DIR/prebuilt/driver/chip/mt5932/wifi_image/mt5932_image_sdio.bin"
    ...
    if [ ! -d "$SOURCE_DIR/middleware/MTK/nbiot" ]; then
        cp -f $SOURCE_DIR/prebuilt/middleware/MTK/nbiot/db/*.dec $OUTPUT/
    fi
    change_cfg_bin_filename $OUTPUT/flash_download.cfg $BIN_FILENAME
fi
