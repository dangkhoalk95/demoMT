...
/**
 * @brief               This function reconfigures the already configured stream and the API needs to be called only in stream open state not in streaming state. The #BT_A2DP_RECONFIGURE_CNF event will be sent to the upper layer with reconfigured stream result.
 * @param[in] handle    is the connection handle of the specified remote device.
 * @param[in] codec_cap    is the codec capabilities to renegotiate with the remote device.
 * @return              #BT_STATUS_SUCCESS, the operation completed successfully.
 *                      #BT_STATUS_FAIL, the operation has failed.
 *                      #BT_STATUS_OUT_OF_MEMORY, out of memory.
 */
...
/**
 * @brief               This function returns the maximum A2DP payload data value in bytes so that the app can send data less than this value.
 * @param[in] handle    is the connection handle of the specified remote device.
