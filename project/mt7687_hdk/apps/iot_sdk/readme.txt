 ...
 * @{
 * @addtogroup mt7687_hdk_apps_iot_sdk iot_sdk
 * @{

@par Overview
  - Example project description
    - This project demonstrates the Wi-Fi connectivity and low power
      consumption features of the LinkIt 7687 HDK through the following:
...
@par Hardware and software environment
  ...
  - HDK switches and pin configuration
    ...
    - J25 set the HDK to either Flash Normal mode or Flash Recovery mode. To
      update the firmware on the LinkIt 7687 HDK:
...
@par Directory contents
  - Source and header files
    ...
    - \b inc/sys_init.h:          Prototype declaration for system init.
    - \b inc/task_def.h:          The configuration of running tasks of the
                                  project.
    - \b inc/wifi_lwip_helper.h:  Prototype declaration for wifi lwip
...
@par Run the examples
  ...
  - Example 2. Wi-Fi access point mode.
    ...
    - Use a handheld device or a laptop computer to connect to the access
      point 'iot_ap'. In the console, the IP address assigned to the the
      handheld device or laptop is:
      \code
      [DHCPD:DBG]lease_ip:10.10.10.2
      \endcode

@par Low power system configuration with tickless feature
  - Tickless is a key feature of IoT devices for low power consumption to
    extend the battery life. For more details about the tickless feature,
    please refer to Airoha IOT Development Platform for RTOS Power Mode
    Developer's Guide.
  - Enable or disable the feature The tickless feature is configured in the
    FreeRTOSConfig.h header file and the parameters can be adjusted as
    follows:
    - configUSE_TICKLESS_IDLE For this configuration, 3 different values can
      be used. 0  Disable tickless. 1  Enable tickless, and use the FreeRTOS
      pre-defined implementation. This will only configure Cortex-M4 into WFI
      mode. No other MediaTek hardware can be controlled for power saving. 2
      Enable tickless, and use the MediaTek tickless porting method. All
      MediaTek hardware, including Cortex-M4, can be set with the minimum
      power consumption.
    - configTICKLESS_DYNAMIC_ENABLE Define this configuration to dyanmically
      enable or disable tickless feature with a CLI command. Example 1.
      Disable tickless.
      \code
      lp tick 0
      \endcode Example 2. Enable tickless.
      \code
      lp tick 1
      \endcode
*/
