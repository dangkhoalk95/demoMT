...
static void gpio_configure_pull_state_example()
{
    ...
    ret_pinmux_status = hal_pinmux_set_function(HAL_GPIO_4, HAL_GPIO_4_GPIO4);
    ...
    /* Set GPIO as input.*/
    ret = hal_gpio_set_direction(HAL_GPIO_4, HAL_GPIO_DIRECTION_INPUT);
    if (HAL_GPIO_STATUS_OK != ret) {
    ...
    ret = hal_gpio_pull_up(HAL_GPIO_4);
    if (HAL_GPIO_STATUS_OK != ret) {
    ...
    ret = hal_gpio_get_input(HAL_GPIO_4, &data_pull_up);
    if (HAL_GPIO_STATUS_OK != ret) {
    ...
    ret = hal_gpio_pull_down(HAL_GPIO_4);
    if (HAL_GPIO_STATUS_OK != ret) {
    ...
    /* Read the input data of the pin for further validation.*/
    ret = hal_gpio_get_input(HAL_GPIO_4, &data_pull_down);
    if (HAL_GPIO_STATUS_OK != ret) {
        printf("hal_gpio_get_input failed\r\n");
        hal_gpio_deinit(HAL_GPIO_4);
        return;
    ...
    ret = hal_gpio_deinit(HAL_GPIO_4);;
    if (HAL_GPIO_STATUS_OK != ret) {
