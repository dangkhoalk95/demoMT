...
*@return None.
*/
static bool spi_master_transfer_data_two_boards_example(void)
{
    uint32_t i = 0;
    ...
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_set_dummy_bits(SPIM_TEST_PORT, 0)) {
        printf("SPI master set dummy bits failed\r\n");
        return false;
    }
    ...
    /* Step4: SPI Master starts to send power_on command to SPI slave */
    ...
    /* Step5: SPI Master starts to send config write command to SPI slave */
    ...
    /* Step6: Config SPI master to use quad mode for write command */
    printf("SPI Master starts to send WR command to SPI slave\r\n");
    hal_spi_master_set_mode(SPIM_TEST_PORT, HAL_SPI_MASTER_SINGLE_MODE);
    hal_spi_master_set_command_bytes(SPIM_TEST_PORT, 1);
    ...
    /* Step8: SPI Master starts to send cfg_rd command to SPI slave */
    ...
    /* Step9: Config SPI master to use quad mode for rd command */
    ...
    /* Step10: SPI Master check whether the data received match the data sent before. */
    ...
    /* Step11: SPI Master starts to send PWOFF command to SPI slave */
    ...
    /* Step12: De-initialize SPI master and the GPIO. */
...
int main(void)
{
    ...
    /* Add your application code here. */
    ret = spi_master_transfer_data_two_boards_example();
