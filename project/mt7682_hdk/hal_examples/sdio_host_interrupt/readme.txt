 ...
 * @{
 * @addtogroup mt7682_hdk_hal_examples_sdio_host_interrupt sdio_host_interrupt
 * @{
...
@par Overview
  - Example description
    - This example is a reference application to demonstrate how to use SDIO
      ...
      The example is the SDIO master part.
...
@par Directory contents
  - Source and header files
    ...
    - \b src/sdio_hal.c:        SDIO function read/write program.
    - \b src/sdio_test.c:       SDIO test API program.
    - \b src/slt_sdio_host.c:   SDIO test program.
    ...
    - \b inc/sdio_hal.h:        SDIO function read/write header file.
    - \b inc/sdio_reg.h:        MT7682 SDIO slave host domain register map file.
  ...
  - Project configuration files using IAR
    - \b EWARM/sdio_host_interrupt.ewd:
                           IAR project options. Contains the settings for the
                           debugger.
    - \b EWARM/sdio_host_interrupt.ewp:
                           IAR project file. Contains the project structure in
                           XML format.
    - \b EWARM/sdio_host_interrupt.eww:
                           IAR workspace file. Contains project information.
    - \b EWARM/flash.icf:  Linker script.

@par Run the example
  - Build the example project for Master with a command "./build.sh
    mt7682_hdk sdio_host_interrupt bl" from the SDK root folder and download
    the binary file to the LinkIt 7682 HDK that operates as SDIO Master.
  - Build the example project for Slave with a command "./build.sh mt7682_hdk
    sdio_slave_interrupt bl" from the SDK root folder and download the
    binary file to the 7682 HDK that operates as SDIO Slave.
