...
int main(void)
{
    ...
    /* Initialize wifi stack and register wifi init complete event handler,
        ...
        */
    LOG_I(nghttp2_client_main, "Call wifi init");
    wifi_init(&config, NULL);

    LOG_I(nghttp2_client_main, "wifi init ok, call network init");
    /* Tcpip stack and net interface initialization,  dhcp client, dhcp server process initialization. */
    lwip_network_init(config.opmode);
    lwip_net_start(config.opmode);

    LOG_I(nghttp2_client_main, "network init ok, call xTaskCreate");
    ...
    LOG_I(nghttp2_client_main, "xTaskCreate ok, call SysInitStatus_Set");
    /* Call this function to indicate the system initialize done. */
    SysInitStatus_Set();

    LOG_I(nghttp2_client_main, "SysInitStatus_Set ok");
    /* Start the scheduler. */
    vTaskStartScheduler();
