...
/* Create the log control block as user wishes. Here we use 'SLEEP' as module name.
...
static void enter_power_saving_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        LOG_I(SLEEP_MANAGER, "example project test success and Power off.\n");
...
int main(void)
{
     ...
     */
#ifdef WITHOS
    LOG_I(SLEEP_MANAGER, "This example run with OS \n");
    ...
    vTaskStartScheduler();
#else //non-os
    LOG_I(SLEEP_MANAGER, "This example run with non-OS \n");
    __asm volatile("cpsid i");
    LOG_I(SLEEP_MANAGER, "Example:Enter Deep Sleep.\n");
    hal_sleep_manager_set_sleep_time(example_sleep_time);
    hal_sleep_manager_enter_sleep_mode(HAL_SLEEP_MODE_DEEP_SLEEP);
