...
{
    static int8_t sentence_buf[GNSS_RECIEVE_TEMP_BUF + 1];
...
void gnss_app_send_cmd(int8_t* buf, int32_t buf_len)
{
    ...
    if (buf_len + 7 > GNSS_NEMA_SENTENCE_BUF_LEN) {
        return;
    }
...
// You can disable it when you don't need it.
    ...
    switch (type) {
        ...
        case GNSS_NOTIFICATION_TYPE_DEBUG_INFO:
            ...
            // This feature is used to enable send debug log to smart phone via BT.
            #ifdef GNSS_SUPPORT_BT_BRIDGE
            gnss_bt_bridge_send_data((uint8_t*)debug_info->debug_data, (uint32_t)debug_info->length);
            #endif
            return;
