...
/*****************************************************************************
* FUNCTION
*  ble_ancs_remove_all_from_notification_list
* DESCRIPTION

* Remove all notification from the link list.
* PARAMETERS
*  
* RETURNS
*  void
...
*****************************************************************************/
void ble_ancs_remove_all_from_notification_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    LOG_I(ANCS, "[ANCSapp]notif_num = %d!\r\n", p_ancs_app->notif_num);
    return;
}
...
void ble_ancs_deal_with_new_notification(bt_handle_t connection_handle, ble_ancs_event_notification_t *notif)
{
    ...
    if (notif->event_id == BLE_ANCS_EVENT_ID_NOTIFICATION_REMOVED) {
        ...
        if (p_notif) {
            ...
            if (NULL != p_ancs_app->curr_notif && 
                0 == memcmp(p_ancs_app->curr_notif->notif.notif_uid, notif->notification_uid, 4)) {
                p_ancs_app->curr_notif = NULL;
            }
...
void ble_ancs_message_handler(ble_ancs_event_t *ancs_evt)
{
    ...
    switch (ancs_evt->evt_type) {
        ...
        case BLE_ANCS_EVENT_APP_ATTRIBUTE: 
        {
            ...
            break;
        case BLE_ANCS_EVENT_PARSE_RECEIVE_DATA_ERROR: 
        {
            LOG_I(ANCS, "[ANCSapp]ERROR happen!\r\n");
            vTaskDelay(1000);
            ble_ancs_remove_from_notification_list(p_ancs_app->curr_notif);
            p_ancs_app->curr_notif = NULL;
            ble_ancs_get_notification_information();
