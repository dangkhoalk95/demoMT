...
static void bt_sink_app_voice_prompt_int_play(uint8_t tone_idx);
#endif
...
{
#ifdef  __BT_SINK_SRV_VOICE_PROMPT__
    bt_sink_app_voice_prompt_report("power off flag: %d", g_bt_sink_app_voice_prompt_power_off);
    if (!g_bt_sink_app_voice_prompt_power_off) {
        bt_sink_app_voice_prompt_int_play(tone_idx);
    }
...
void bt_sink_app_voice_prompt_event_handle(bt_sink_srv_action_t action)
{
	...
	#ifdef  __BT_SINK_SRV_VOICE_PROMPT__
    bt_sink_app_voice_prompt_int_play(mix_tone);
	#endif
    return;
}
