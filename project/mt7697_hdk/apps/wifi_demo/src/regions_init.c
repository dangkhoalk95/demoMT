...
const memory_region_type memory_regions[] =
{
    ...
    {"sram",  __SRAM_segment_start__, __bss_end__, 1},
...
{
    *pxBase = ((uint32_t)__bss_end__ + 0x20) & ~(0x20 - 1);  /* align up to 32Byte */
    *pxLen = (uint32_t)(((uint32_t)__StackLimit - *pxBase) ) & ~(0x20 - 1);  /* swla buffer area is free ram + swla  reserve area */
}
