...
#else /* !MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE */
...
void hex_dump_data(const char *data, int length)
{
    int index = 0;
    for (index = 0; index < length; index++) {
        platform_printf("%02X", (int)(data[index]));
        if ((index + 1) % 16 == 0) {
            platform_printf("\r\n");
            continue;
        }
...
void process_log_message(const log_message_t *log_message)
{
    if (syslog_drop_count) {
        UBaseType_t uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR();;
        platform_printf("\t\t>>> log drop count = %u\r\n\n", (unsigned int)syslog_drop_count);
        syslog_drop_count = 0;
        portCLEAR_INTERRUPT_MASK_FROM_ISR( uxSavedInterruptStatus );
    }
