...
static int32_t bt_sink_srv_mp3_read_data(void)
{
                ...
                if ( mp3_ctx->supportAws ) {
                    ...
                    if (remain_byte > 0) {
                        ...
                        if (dev->op & BT_SINK_SRV_MUSIC_OP_CODEC_OPEN) {
                            // small file and has ID3 footer
                            if ((dev->flag & BT_SINK_SRV_MUSIC_FLAG_MP3_PLAYING) == false) {
                                bt_sink_srv_report("[MP3] short end remain %d, read %d\r\n", remain_byte, read_byte);
                                buff_mgr_write_data_done(read_byte);
                                mp3_ctx->threshold = buff_mgr_get_data_count() - 1;
                                return 0;
                            // eof change track
                            } else if (bt_sink_srv_mp3_eof_file(&g_aud_fdst)) {
    ...
    if (bt_sink_srv_mp3_eof_file(&g_aud_fdst)) {
        ...
        if (dev->flag & BT_SINK_SRV_MUSIC_FLAG_MP3_PLAYING) {
            ...
            bt_sink_srv_mp3_local_change_track(MP3_NEXT_TRACK);
            ret = -5;
        } else if (dev->op & BT_SINK_SRV_MUSIC_OP_CODEC_OPEN) {
            bt_sink_srv_report( "[Audply] play short tone\r\n");
            ret = 0;
        } else {
            bt_sink_srv_mp3_next_track();
            ret = -6;
        ...
        if (bt_sink_srv_mp3_over_threshold() && g_file_med_hd) {
        ...
        #endif
            {
                ...
                if ((bt_sink_srv_mp3_read_data() != 0) && 
                        (bt_sink_srv_mp3_eof_file(&g_aud_fdst) == false)) {
