...
#define PIN_EXPIRES BIT(1)
    ...
    kal_uint32 expiration; // in second
};
...
struct wps_pbc_session {
    ...
    kal_uint32  timestamp; // in second
};
...
//    now = kal_ticks_to_secs(now);
    ...
    while (pbc) {
        if (now > pbc->timestamp + WPS_PBC_WALK_TIME) {
            prev->next = NULL;
...
static int wps_build_wps_state(struct wps_context *wps, struct wpabuf *msg)
{
    ...
    // wps->wps_state);
//  kal_trace(DBG_WPS,INFO_WPS_BUILD_WPS_STATE, wps->wps_state);
...
{
    struct wps_registrar *reg = SUPC_ALLOC_BUFF(sizeof(struct wps_registrar));
    if (reg == NULL) {
        return NULL;
    }
    memset(reg, 0, sizeof(struct wps_registrar));
                          ...
                          const u8 *pin, size_t pin_len, int timeout)
{
    ...
    if (timeout) {
        ...
        kal_get_time(&now);
//        p->expiration = kal_ticks_to_secs(now);
        p->expiration = now / 1000;
    ...
    //timeout);
    wpa_hexdump(MSG_DEBUG, "WPS: UUID", (char *)uuid, WPS_UUID_LEN);
    wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: PIN", (char *)pin, pin_len);
...
#endif
    supc_stop_wps_reg_set_selected_timer();
    supc_start_wps_registrar_pbc_timer(WPS_PBC_WALK_TIME);
    return 0;
}
...
//    now = kal_ticks_to_secs(now);
    ...
    while (pin) {
        if ((pin->flags & PIN_EXPIRES) &&
                time_before(&pin->expiration, &now)) {
            if (prev == NULL) {
                reg->pins = pin->next;
            } else {
                prev->next = pin->next;
...
static int wps_build_r_hash(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: R-S1", (char *)wps->snonce, WPS_SECRET_NONCE_LEN);
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: R-Hash1", (char *)hash, SHA256_MAC_LEN);
    ...
    addr[1] = wps->psk2;
    hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 4, addr, len, hash);
    wpa_hexdump(MSG_DEBUG, "WPS: R-Hash2", (char *)hash, SHA256_MAC_LEN);
...
static int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
        ...
        if (hostapd_get_rand(r, sizeof(r)) < 0) {
            return -1;
        }
        os_free(wps->new_psk);
        wps->new_psk = supc_base64_encode(r, sizeof(r), &wps->new_psk_len);
        if (wps->new_psk == NULL) {
            return -1;
        }
        ...
        wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated passphrase",
                              (char *)wps->new_psk, wps->new_psk_len);
        ...
        if (hostapd_get_rand(wps->new_psk, wps->new_psk_len) < 0) {
            os_free(wps->new_psk);
            wps->new_psk = NULL;
            return -1;
        }
...
static struct wpabuf *wps_build_m2(struct wps_data *wps)
{
    ...
    if (hostapd_get_rand(wps->nonce_r, WPS_NONCE_LEN) < 0) {
        return NULL;
    }
...
static int wps_process_uuid_e(struct wps_data *wps, const u8 *uuid_e)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: UUID-E", (char *)wps->uuid_e, WPS_UUID_LEN);
...
static int wps_process_e_hash1(struct wps_data *wps, const u8 *e_hash1)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: E-Hash1", (char *)wps->peer_hash1, WPS_HASH_LEN);
...
static int wps_process_e_hash2(struct wps_data *wps, const u8 *e_hash2)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: E-Hash2", (char *)wps->peer_hash2, WPS_HASH_LEN);
...
static int wps_process_e_snonce1(struct wps_data *wps, const u8 *e_snonce1)
{
    ...
    wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce1", (char *)e_snonce1,
                    WPS_SECRET_NONCE_LEN);
...
static int wps_process_e_snonce2(struct wps_data *wps, const u8 *e_snonce2)
{
    ...
    wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce2", (char *)e_snonce2,
                    WPS_SECRET_NONCE_LEN);
...
{
    struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)para_hf;
    struct wps_registrar *reg = NULL;

    SUPC_ASSERT(NULL != wpa_s);
    SUPC_ASSERT(NULL != wpa_s->wps);
    SUPC_ASSERT(NULL != wpa_s->wps->registrar);
    reg = wpa_s->wps->registrar;
