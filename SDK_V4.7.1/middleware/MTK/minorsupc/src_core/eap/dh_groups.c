/*
 * Diffie-Hellman groups
 * Copyright (c) 2007, Jouni Malinen <j@w1.fi>
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * Alternatively, this software may be distributed under the terms of BSD
 * license.
 *
 * See README and COPYING for more details.
 */

#ifndef MTK_WIFI_CHIP_USE_MT5931
#pragma import(__use_no_semihosting_swi)
#endif

//#include "kal_release.h"
#include "kal_general_types.h"
#include "kal_public_defs.h"
#include "kal_public_api.h"
//#include "supc_trc.h"
#include "wndrv_supc_types.h"
#include "supc_common.h"
#include "wpa.h"

#include "wpa_supplicant_i.h"

#include "supc_wpa_common.h"
#include "config_ssid.h"
#include "supc_config.h"
#include "eap.h"
#include "wpa_supplicant_i.h"
#include "wpabuf.h"

#include "crypto.h"
#include "dh_groups.h"

#ifdef ALL_DH_GROUPS
/* RFC 4306, B.1. Group 1 - 768 Bit MODP
 * Generator: 2
 * Prime: 2^768 - 2 ^704 - 1 + 2^64 * { [2^638 pi] + 149686 }
 */
static const u8 dh_group1_generator[1] = { 0x02 };
static const u8 dh_group1_prime[96] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68, 0xC2, 0x34,
    0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74,
    0x02, 0x0B, 0xBE, 0xA6, 0x3B, 0x13, 0x9B, 0x22,
    0x51, 0x4A, 0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD,
    0xEF, 0x95, 0x19, 0xB3, 0xCD, 0x3A, 0x43, 0x1B,
    0x30, 0x2B, 0x0A, 0x6D, 0xF2, 0x5F, 0x14, 0x37,
    0x4F, 0xE1, 0x35, 0x6D, 0x6D, 0x51, 0xC2, 0x45,
    0xE4, 0x85, 0xB5, 0x76, 0x62, 0x5E, 0x7E, 0xC6,
    0xF4, 0x4C, 0x42, 0xE9, 0xA6, 0x3A, 0x36, 0x20,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
};
/*modify remove group1 from ALL_DH_GROUPS compile option*/
/* RFC 4306, B.2. Group 2 - 1024 Bit MODP
 * Generator: 2
 * Prime: 2^1024 - 2^960 - 1 + 2^64 * { [2^894 pi] + 129093 }
 */
static const u8 dh_group2_generator[1] = { 0x02 };
static const u8 dh_group2_prime[128] = {
    
};

#endif /* ALL_DH_GROUPS */

/* RFC 3526, 2. Group 5 - 1536 Bit MODP
 * Generator: 2
 * Prime: 2^1536 - 2^1472 - 1 + 2^64 * { [2^1406 pi] + 741804 }
 */
static const u8 dh_group5_generator[1] = { 0x02 };
static const u8 dh_group5_prime[GROUP_5_PRIME_LEN] = {
    
};

#ifdef ALL_DH_GROUPS

/* RFC 3526, 3. Group 14 - 2048 Bit MODP
 * Generator: 2
 * Prime: 2^2048 - 2^1984 - 1 + 2^64 * { [2^1918 pi] + 124476 }
 */
static const u8 dh_group14_generator[1] = { 0x02 };
static const u8 dh_group14_prime[256] = {
    
};

/* RFC 3526, 4. Group 15 - 3072 Bit MODP
 * Generator: 2
 * Prime: 2^3072 - 2^3008 - 1 + 2^64 * { [2^2942 pi] + 1690314 }
 */
static const u8 dh_group15_generator[1] = { 0x02 };
static const u8 dh_group15_prime[384] = {
    
};

/* RFC 3526, 5. Group 16 - 4096 Bit MODP
 * Generator: 2
 * Prime: 2^4096 - 2^4032 - 1 + 2^64 * { [2^3966 pi] + 240904 }
 */
static const u8 dh_group16_generator[1] = { 0x02 };
static const u8 dh_group16_prime[512] = {
    
};

/* RFC 3526, 6. Group 17 - 6144 Bit MODP
 * Generator: 2
 * Prime: 2^6144 - 2^6080 - 1 + 2^64 * { [2^6014 pi] + 929484 }
 */
static const u8 dh_group17_generator[1] = { 0x02 };
static const u8 dh_group17_prime[768] = {
    
};

/* RFC 3526, 7. Group 18 - 8192 Bit MODP
 * Generator: 2
 * Prime: 2^8192 - 2^8128 - 1 + 2^64 * { [2^8062 pi] + 4743158 }
 */
static const u8 dh_group18_generator[1] = { 0x02 };
static const u8 dh_group18_prime[1024] = {
    
};

#endif /* ALL_DH_GROUPS */


#define DH_GROUP(id) \
{ id, dh_group ## id ## _generator, sizeof(dh_group ## id ## _generator), \
dh_group ## id ## _prime, sizeof(dh_group ## id ## _prime) }


static struct dh_group dh_groups[] = {
    DH_GROUP(5),
#ifdef ALL_DH_GROUPS
    DH_GROUP(1),
    DH_GROUP(2),
    DH_GROUP(14),
    DH_GROUP(15),
    DH_GROUP(16),
    DH_GROUP(17),
    DH_GROUP(18)
#endif /* ALL_DH_GROUPS */
};

#define NUM_DH_GROUPS (sizeof(dh_groups) / sizeof(dh_groups[0]))


const struct dh_group *dh_groups_get(int id)
{
    size_t i;

    for (i = 0; i < NUM_DH_GROUPS; i++) {
        if (dh_groups[i].id == id) {
            return &dh_groups[i];
        }
    }
    return NULL;
}


/**
 * dh_init - Initialize Diffie-Hellman handshake
 * @dh: Selected Diffie-Hellman group
 * @priv: pointer to che_ctx_dh object stored in wps_data
 * Returns: Diffie-Hellman public value
 */
struct wpabuf *dh_init(const struct dh_group *dh, struct wpabuf **privkey)
{
    struct wpabuf *pv;
    kal_uint32 pv_len;
    kal_uint32 priv_len;
    STCHE che;

    if (dh == NULL) {
        return NULL;
    }
#if 0
    //wpabuf_free(*priv);
    //*priv = wpabuf_alloc(dh->prime_len);

    if (*priv == NULL) {
        return NULL;
    }

    if (hostapd_get_rand(wpabuf_put(*priv, dh->prime_len), dh->prime_len)) {
        wpabuf_free(*priv);
        *priv = NULL;
        return NULL;
    }

    if (os_memcmp(wpabuf_head(*priv), dh->prime, dh->prime_len) > 0) {
        /* Make sure private value is smaller than prime */
        *(wpabuf_mhead_u8(*priv)) = 0;
    }
    wpa_hexdump_buf_key(MSG_DEBUG, "DH: private value", *priv);

    pv_len = dh->prime_len;
    pv = wpabuf_alloc(pv_len);
    if (pv == NULL) {
        return NULL;
    }

    if (crypto_mod_exp(dh->generator, dh->generator_len,
                       wpabuf_head(*priv), wpabuf_len(*priv),
                       dh->prime, dh->prime_len, wpabuf_mhead(pv),
                       &pv_len) < 0) {
        wpabuf_free(pv);
        wpa_printf(MSG_INFO, "DH: crypto_mod_exp failed");
        return NULL;
    }
#endif
    // use MAUI che lib's api

    che_init(&che, CHE_DH);

    che_process(&che, CHE_DH, CHE_MODE_NULL, DH_INIT,
                NULL, NULL, 0, KAL_TRUE);

    che_key_action(&che, CHE_SET_DH_P, (kal_uint8 *)dh->prime, dh->prime_len);
    //wpa_printf(MSG_DEBUG, "DH-Group %d, prime_len=%d", dh->id, dh->prime_len);
    //wpa_hexdump(MSG_DEBUG, "DH-prime", dh->prime, dh->prime_len);

    che_key_action(&che, CHE_SET_DH_G, (kal_uint8 *)dh->generator, dh->generator_len);
    //wpa_printf(MSG_DEBUG, "DH-Group %d, generator_len=%d", dh->id, dh->generator_len);
    //wpa_hexdump(MSG_DEBUG, "DH-generator", dh->generator, dh->generator_len);

    che_process(&che, CHE_DH, CHE_MODE_NULL, DH_MAKE_PARAMS,
                NULL, NULL, 0, KAL_TRUE);
    //wpa_printf(MSG_DEBUG, "generate DH key OK");

    che_key_action(&che, CHE_GET_DH_SELF_PUBLIC_LEN,
                   (void *)&pv_len, sizeof(kal_uint32));
    /*store local public key*/
    pv = wpabuf_alloc(pv_len);
    if (pv == NULL) {
        goto fail;
    }
    che_key_action(&che, CHE_GET_DH_SELF_PUBLIC,
                   (kal_uint8 *)wpabuf_head_u8(pv), pv_len);
    wpabuf_put(pv, pv_len);
    wpa_hexdump_buf(MSG_DEBUG, "DH: public value", pv);

    /*store local private key*/
    che_key_action(&che, CHE_GET_DH_PRIVATE_LEN, (void *)&priv_len, sizeof(kal_uint32));
    *privkey = wpabuf_alloc(priv_len);
    if (*privkey == NULL) {
        //wpa_printf(MSG_DEBUG, "DH: fail to get local prive key");
//      kal_trace(DBG_EAP_METHOD,INFO_DH_FAIL_GET_LOCAL_PRVI_KEY);
        wpabuf_free(pv);
        pv = NULL;
        goto fail;
    }
    che_key_action(&che, CHE_GET_DH_PRIVATE,
                   (kal_uint8 *)wpabuf_head_u8(*privkey), priv_len);
    wpabuf_put(*privkey, priv_len);
    wpa_hexdump_buf(MSG_DEBUG, "DH: local private value", *privkey);

fail:

    che_process(&che, CHE_DH, CHE_MODE_NULL,
                DH_FREE, NULL, NULL, 0, KAL_TRUE);
    che_deinit(&che);
    return pv;

}


/**
 * dh_derive_shared - Derive shared Diffie-Hellman key
 * @peer_public: Diffie-Hellman public value from peer
 * @own_private: Diffie-Hellman private key from dh_init()
 * @dh: Selected Diffie-Hellman group
 * Returns: Diffie-Hellman shared key
 */
struct wpabuf *dh_derive_shared(const struct wpabuf *peer_public,
                                struct wpabuf *privkey, struct wpabuf *local_public,
                                const struct dh_group *dh)
{
    struct wpabuf *shared;
    kal_uint32 shared_len;
    STCHE che;
    if (dh == NULL || peer_public == NULL || privkey == NULL) {
        return NULL;
    }
#if 0
    shared_len = dh->prime_len;
    shared = wpabuf_alloc(shared_len);
    if (shared == NULL) {
        return NULL;
    }
    if (crypto_mod_exp(wpabuf_head(peer_public), wpabuf_len(peer_public),
                       wpabuf_head(own_private), wpabuf_len(own_private),
                       dh->prime, dh->prime_len,
                       wpabuf_put(shared, shared_len), &shared_len) < 0) {
        wpabuf_free(shared);
        wpa_printf(MSG_INFO, "DH: crypto_mod_exp failed");
        return NULL;
    }
#endif
    che_init(&che, CHE_DH);
    che_process(&che, CHE_DH, CHE_MODE_NULL, DH_INIT, NULL, NULL, 0, KAL_TRUE);

    /* set the P, G, the stored private and public */
    che_key_action(&che, CHE_SET_DH_P, (kal_uint8 *) dh->prime, dh->prime_len);
    che_key_action(&che, CHE_SET_DH_G, (kal_uint8 *)dh->generator, dh->generator_len);

    che_key_action(&che, CHE_SET_DH_SELF_PUBLIC,
                   (kal_uint8 *) wpabuf_head_u8(local_public), wpabuf_len(local_public));
    che_key_action(&che, CHE_SET_DH_PRIVATE,
                   (kal_uint8 *)wpabuf_head_u8(privkey), wpabuf_len(privkey));

    // Set the peer public key (A) in binary format
    che_key_action(&che, CHE_SET_DH_PEER_PUBLIC,
                   (kal_uint8 *)wpabuf_head_u8(peer_public), wpabuf_len(peer_public));

    /* calculate the shared secret: K */
    che_process(&che, CHE_DH, CHE_MODE_NULL, DH_CALC_SECRET,
                NULL, NULL, 0, KAL_TRUE);

    /* get the shared secret key */
    che_key_action(&che, CHE_GET_DH_SECRET_LEN,
                   (void *)&shared_len, sizeof(kal_uint32));

    shared = wpabuf_alloc(shared_len);
    if (shared == NULL) {
        goto fail;
    }
    che_key_action(&che, CHE_GET_DH_SECRET,
                   (kal_uint8 *)wpabuf_head_u8(shared), shared_len);
    wpabuf_put(shared, shared_len);
    wpa_hexdump_buf_key(MSG_DEBUG, "DH: shared key", shared);
fail:
    che_process(&che, CHE_DH, CHE_MODE_NULL,
                DH_FREE, NULL, NULL, 0, KAL_TRUE);
    che_deinit(&che);
    return shared;
}
