 ...
 *                           // Keep the current AVRCP application state and context till the BT_AVRCP_OPERATION_STATE_RELEASED event is confirmed.
 *                       }
 *                   }
 *                   break;
 *               }
 ...
 *
 *    @endcode
 *  - Step 3. Mandatory, disconnect the remote device, as shown in the figure titled as "AVRCP connection release" in the Bluetooth developer's guide under the [sdk_root]/doc folder.
 *   - Sample code:
 *    @code
...
#define BT_AVRCP_EVENT_NOTIFICATION_IND           (BT_MODULE_AVRCP | 0x0008)   /**< The event notification indication event indicates the notification to register an event 
                                                                                    after calling #bt_avrcp_register_notification(). The structure for this event is defined as #bt_avrcp_event_notification_t. */
#define BT_AVRCP_REQUEST_CONTINUING_CNF           (BT_MODULE_AVRCP | 0x0009)   /**< The request continuing confirmation event is only triggered after calling #bt_avrcp_request_continuing_response() has failed. The structure for this event is defined as #bt_avrcp_metadata_error_response_t. */
...
#define BT_AVRCP_ELEMENT_METADATA_IND        (BT_MODULE_AVRCP | 0x0014)   /**< The event indicates element metadata indication command from CT is received from CT. */
#define BT_AVRCP_GET_CAPABILITY_IND        (BT_MODULE_AVRCP | 0x0015)   /**< The event indicates get capability command indication from CT is received. */
#define BT_AVRCP_ELEMENT_METADATA_CONT_IND        (BT_MODULE_AVRCP | 0x0016)   /**< The event indicates element metadata CONTINUATION indication command from CT is received */
#define BT_AVRCP_CONTINUATION_ABORT_IND        (BT_MODULE_AVRCP | 0x0017)   /**< abort the CONTINUATION from CT is received */
...
#define BT_AVRCP_ROLE_CT             0          /**< Response code for AVRCP command. TG sends the notification that the state of TG has changed. */
#define BT_AVRCP_ROLE_TG             1          /**< Response code for AVRCP command. TG has accepted the requested command but cannot return information within 100 milliseconds. */
...
#define    BT_AVRCP_EVENT_AVAILABLE_PLAYERS_CHANGED 0x0a	/**< Available Players Changed Notification: event allows the CT to be informed if a new player becomes available to be addressed (for instance started, or installed)*/
...
/**
* @brief systemstatus for registered event BT_AVRCP_EVENT_SYSTEM_STATUS_CHANGED.
*/

#define    BT_AVRCP_STATUS_SYSTEM_POWER_ON 0x00      /**< player is on */
#define    BT_AVRCP_STATUS_SYSTEM_POWER_OFF 0x01                /**< player is off */
#define    BT_AVRCP_STATUS_SYSTEM_UNPLUGGED 0x02            /**< player is not connected to bluetooth*/
...
#define    BT_AVRCP_STATUS_BATT_WARNING 0x01                /**< unable to operate soon. Is provided when the battery level is going down.*/
#define    BT_AVRCP_STATUS_BATT_CRITICAL 0x02        /**< Cannot operate any more. Is provided when the battery level is goingdown.  */
#define    BT_AVRCP_STATUS_BATT_EXTERNAL 0x03          /**< Plugged to external power supply */
#define    BT_AVRCP_STATUS_BATT_FULL_CHARGE 0x04         /**< when the device is completely charged from the external power supply */
             
/**
* @brief playstatus for registered event BT_AVRCP_EVENT_PLAYBACK_STATUS_CHANGED.
*/

#define    BT_AVRCP_STATUS_PLAY_STOPPED 0x00      /**< Player is stopped */
#define    BT_AVRCP_STATUS_PLAY_PLAYING 0x01                /**< song playing */
#define    BT_AVRCP_STATUS_PLAY_PAUSED 0x02            /**< song paused */
#define    BT_AVRCP_STATUS_PLAY_FWD_SEEK 0x03          /**< Forward seek */
#define    BT_AVRCP_STATUS_PLAY_REV_SEEK 0x04         /**< reverse seek */
#define    BT_AVRCP_STATUS_PLAY_ERROR 0xFF            /**< error  */
...
#define BT_AVRCP_PDU_ID_LIST_APP_SETTING_VALUES 0X12    /**< The PDU ID for  */
...
#define BT_AVRCP_PDU_ID_GET_PLAYER_APP_SETTING_ATT_TXT 0X15    /**< The PDU ID for  */
#define BT_AVRCP_PDU_ID_GET_PLAYER_APP_SETTING_VALUE_TXT 0X16    /**< The PDU ID for  */
#define BT_AVRCP_PDU_ID_INFORM_DISPLAYBLE_CHAR_SET 0X17    /**< The PDU ID for  */
#define BT_AVRCP_PDU_ID_INFORM_BATTERY_STATUS_OF_CT 0X18    /**< The PDU ID for  */
#define BT_AVRCP_PDU_ID_GET_ELEMENT_ATTRIBUTES 0x20         /**< The PDU ID for #bt_avrcp_get_element_attributes(). */ 
#define BT_AVRCP_PDU_ID_GET_PLAY_STATUS 0x30/**< The PDU ID for #bt_avrcp_get_element_attributes(). */ 
...
#define BT_AVRCP_PDU_ID_SET_ADDRESSED_PLAYER  0x60          /**< MediaPlayerSelection : The PDU ID for . */
#define BT_AVRCP_PDU_ID_GET_FOLDER_ITEMS_LIST   0x71          /**< MediaPlayerSelection: The PDU ID for . */
#define BT_AVRCP_PDU_ID_GET_TOTAL_NO_OF_ITEMS   0x75          /**< MediaPlayerSelection: The PDU ID for . */
...
#define BT_AVRCP_CAPABILITY_EVENTS_SUPPORTED 0x3 /* use this to indicate capability request from CT */
typedef uint8_t bt_avrcp_capability_types_t;



/* max length of individual attribute content i.e. max length of Title, Artist name, Album name, Genre*/
...
typedef struct{
    ...
    uint8_t value_id;                                       /**< The corresponding player application setting value ID. */
...
typedef struct{
	uint32_t handle;                                        /**< The connection handle of the AVRCP. */
	uint8_t number;                                        /**< If NumAttributes is set to zero, all attribute information shall be returned, else attribute information for the specified attribute IDs shall be returned by the TG*/
...
typedef struct{
	...
	/* for big data PDUs: Metadata Attributes for Current Media Item,Player Application Settings
	...
	BT_AVRCP_ELEMENT_ATTRIBUTE_MAX_DATA_SIZE 400 //size of attributes combined:Title, Artist name, Album name, Genre
	...
	*/
	/* max AVRCP PDU len */
	/* Fill BT_AVRCP_MEDIA_ATTRIBUTE_TITLE as first attribute in packet/data in attribute_value []*/
...
/**
 * @brief                 This function init the avrcp context with the role we want to follow like ct or target
...
/**
 * @brief                 This function sends notifications to the remote device.// BT_AVRCP_EVENT_TRACK_CHANGED, BT_AVRCP_EVENT_TRACK_REACHED_END etc.
 *                        
 * @param[in] handle      is the AVRCP connection handle for the specified remote device.
 * @param[in] data     	  notification data to be transfered to the remote device: 
 * @return                #BT_STATUS_SUCCESS, successfully responded to the connection request. #BT_STATUS_AVRCP_INVALID_PARAMETER, the handle or address is NULL. 
 */
//bt_status_t bt_avrcp_send_notification_t (uint32_t handle, bt_avrcp_event_notification_t *data);
...
/**
 * @brief                 This function metadata attributes to the remote device for some specific element(s).
 *                        // bt_avrcp_media_attribute_t : 0x01 : Title, 0x02 : Artist Name, 0x3 : Album Name etc.
 * @param[in] handle      is the AVRCP connection handle for the specified remote device.
 * @param[in] rsp_data    metadata attributes to be transfered/sent to the remote device: 
 * @return                #BT_STATUS_SUCCESS, successfully responded to the connection request. #BT_STATUS_AVRCP_INVALID_PARAMETER, the handle or address is NULL. 
 */
 bt_status_t
 ...
 * @param[in] handle      is the AVRCP connection handle for the specified remote device.
 * @param[in] rsp_data    device/profile capability attributes to be transfered/sent to the remote device: 
 * @return                #BT_STATUS_SUCCESS, successfully responded to the connection request. #BT_STATUS_AVRCP_INVALID_PARAMETER, the handle or address is NULL. 
 */
