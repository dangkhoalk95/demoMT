...
static void bt_sink_srv_pbapc_add_number(bt_bd_addr_t *address_p, bt_sink_srv_pbapc_record_t *record)
{
    ...
    if (NULL != record && bt_sink_srv_strlen((char *)record->number) < BT_SINK_SRV_MAX_PHONE_NUMBER
            && bt_sink_srv_cm_get_profile_data(address_p, BT_SINK_SRV_UUID_PBAPC, (void *)stored_data_p)) {
        uint32_t i;
        ...
        bt_sink_srv_cm_set_profile_data(address_p, BT_SINK_SRV_UUID_PBAPC, (void *)stored_data_p);
    }
...
static bool bt_sink_srv_pbapc_search_number(bt_bd_addr_t *address, bt_sink_srv_pbapc_record_t *record)
{
    ...
    if (NULL != record && bt_sink_srv_strlen((char *)record->number) < BT_SINK_SRV_MAX_PHONE_NUMBER) {
        ...
        if (bt_sink_srv_cm_get_profile_data(address, BT_SINK_SRV_UUID_PBAPC, (void *)stored_data_p)) {
            for (i = 0; i < BT_SINK_SRV_PBAPC_STORED_COUNT; i++) {
                if (0 == bt_sink_srv_strnmp((const char *)(((*stored_data_p)[i]).number),
                                            (const char *)(record->number),
                                            BT_SINK_SRV_MAX_PHONE_NUMBER)) {
...
{
    bt_sink_srv_profile_t *bt_sink_srv_pbapc_p = NULL;
    ...
    switch (msg) {
        case BT_PBAPC_CONNECT_CNF: {
            ...
            bt_sink_srv_pbapc_p = (bt_sink_srv_profile_t *)bt_sink_srv_cm_find_profile_by_hanlde(message->handle);

            if (NULL != bt_sink_srv_pbapc_p) {
                if (BT_STATUS_SUCCESS == status) {
                    bt_sink_srv_pbapc_p->state = BT_SINK_SRV_PROFILE_STATE_CONNECTED;
                } else {
                    bt_sink_srv_pbapc_p->state = BT_SINK_SRV_PROFILE_STATE_DISCONNECTED;
                }
            }
        }
        break;
        ...
        case BT_PBAPC_DISCONNECT_IND: {
            ...
            if (NULL != bt_sink_srv_pbapc_p) {
                bt_sink_srv_pbapc_p->state = BT_SINK_SRV_PROFILE_STATE_DISCONNECTED;
                bt_sink_srv_pbapc_p->handle = 0;
                pbapc_context_p = (bt_sink_srv_pbapc_context_t *)bt_sink_srv_pbapc_p->data;
        ...
        case BT_PBAPC_GET_CALLER_NAME_IND: {
            ...
            address_p = (bt_bd_addr_t *)bt_sink_srv_cm_find_address_by_hanlde(message->handle);
            bt_sink_srv_pbapc_p = (bt_sink_srv_profile_t *)bt_sink_srv_cm_find_profile_by_hanlde(message->handle);

            if (NULL != address_p && NULL != bt_sink_srv_pbapc_p) {
                pbapc_context_p = (bt_sink_srv_pbapc_context_t *)bt_sink_srv_pbapc_p->data;
                if (NULL != pbapc_context_p && NULL != pbapc_context_p->pending) {
    ...
    switch (action) {
        case BT_SINK_SRV_ACTION_PROFILE_INIT: {
            ...
            for (i = 0; i < BT_SINK_SRV_CM_MAX_DEVICE_NUMBER; i++) {
                g_sink_srv_pbapc[i].handle = 0;
                g_sink_srv_pbapc[i].uuid = BT_SINK_SRV_UUID_PBAPC;
                g_sink_srv_pbapc[i].state = BT_SINK_SRV_PROFILE_STATE_DISCONNECTED;
                g_sink_srv_pbapc[i].data = &g_sink_srv_pbapc_context[i];
            }
        }
        break;
        ...
        case BT_SINK_SRV_ACTION_PROFILE_DISCONNECT: {
            ...
            if (action_param->profile_connection_mask & BT_SINK_SRV_PROFILE_CONNECTION_TYPE_HFP ||
                action_param->profile_connection_mask & BT_SINK_SRV_PROFILE_CONNECTION_TYPE_A2DP_SINK||
                action_param->profile_connection_mask & BT_SINK_SRV_PROFILE_CONNECTION_TYPE_AVRCP) {
                // no aws connection
                bt_sink_srv_report("[Sink][PBAPC] pbapc profile disconnect");
                bt_sink_srv_pbapc_p = bt_sink_srv_cm_find_profile_by_address(&action_param->bt_addr, BT_SINK_SRV_UUID_PBAPC);
                if (NULL != bt_sink_srv_pbapc_p) {
                    bt_sink_srv_report("[Sink][PBAPC] call pbapc profile disconnect");
                    bt_pbapc_disconnect(bt_sink_srv_pbapc_p->handle);
                }
            }
        }
        break;
...
{
    bt_status_t result = BT_STATUS_FAIL;
    bt_sink_srv_profile_t *bt_sink_srv_pbapc_p =
        bt_sink_srv_cm_find_profile_by_address(address, BT_SINK_SRV_UUID_PBAPC);

    if (NULL != bt_sink_srv_pbapc_p &&
            BT_SINK_SRV_PROFILE_STATE_CONNECTED == bt_sink_srv_pbapc_p->state) {
        bt_sink_srv_pbapc_context_t *pbapc_context_p =
            (bt_sink_srv_pbapc_context_t *)bt_sink_srv_pbapc_p->data;
        if (NULL != pbapc_context_p && NULL == pbapc_context_p->pending) {
            pbapc_context_p->pending = (bt_sink_srv_pbapc_record_t *)bt_sink_srv_memory_alloc(sizeof(bt_sink_srv_pbapc_record_t));
            if (NULL != pbapc_context_p->pending) {
                bt_sink_srv_memcpy((void *)pbapc_context_p->pending->number,
                                   (void *)number,
                                   BT_SINK_SRV_MAX_PHONE_NUMBER);
                pbapc_context_p->user_data = user_data;
                pbapc_context_p->callback = callback;
            }
            result = bt_pbapc_get_caller_name_by_number(bt_sink_srv_pbapc_p->handle, number);
        }
    }
...
bt_status_t bt_sink_srv_pbapc_connect(bt_bd_addr_t *address)
{
    ...
    if (NULL != bt_sink_srv_pbapc_p) {
        bt_sink_srv_pbapc_p->state = BT_SINK_SRV_PROFILE_STATE_CONNECTING;
        status = bt_pbapc_connect(&bt_sink_srv_pbapc_p->handle, (const bt_bd_addr_t *)address);
