...
/* default local music mode */
static bt_sink_srv_music_mode_t g_music_mode = BT_SINK_SRV_MODE_BT_MUSIC;
#endif
...
static void bt_mp3_aws_plh_init(bt_sink_srv_am_id_t aud_id);
static void bt_mp3_aws_plh_deinit(void);
#endif /* __BT_AWS_SUPPORT__ */
...
static void aud_set_open_stream_req_hdlr(bt_sink_srv_am_amm_struct *amm_ptr)
{
    ...
    bt_sink_srv_report("[AudM]open_hdr-id: %d, num: %d, use: %d, b_ptr: 0x%x\n", bAud_id,
        g_aud_id_num, g_rAm_aud_id[bAud_id].use, (unsigned int)Background_ptr);
}
...
static void aud_set_play_stream_req_hdlr(bt_sink_srv_am_amm_struct *amm_ptr)
{
    ...
    if ((recoder_high_t != NULL) && (recoder_high_t->aud_id != bAud_id)) {
    ...
    if (g_prA2dp_sink_handle != NULL) {
        ...
        switch (event_id) {
            ...
            case BT_CODEC_MEDIA_AWS_CHECK_CLOCK_SKEW: {
                ...
                bt_sink_srv_audio_clock_calibrate(g_prA2dp_sink_handle, g_aws_codec_type, sampling_rate);
                break;
            }
        #endif /* __BT_AWS_SUPPORT__ */
...
#ifdef __BT_SINK_SRV_AM_MED_LIST_SUPPORT__
    g_prA2dp_sink_handle->set_get_data_function(g_prA2dp_sink_handle, bt_sink_srv_am_med_mgr_read_data);
    g_prA2dp_sink_handle->set_get_data_count_function(g_prA2dp_sink_handle, bt_sink_srv_am_med_mgr_get_data_count);
#else
...
{
    if (aud_id == g_prCurrent_player->aud_id) {
        bt_mp3_plh_init((bt_media_handle_t *)g_am_mp3_media_handle);
    }
}
...
void bt_sink_srv_skew_test(const char *value)
{
    ...
    bt_sink_srv_report("[Sink][AM]skew_test--value: %s, count: %d\n", value, sample_count);
...
static void am_files_codec_open(bt_sink_srv_am_files_format_t *files_format)
{
    ...
    switch (type) {
        case FILE_MP3: {
            ...
            if (g_am_mp3_media_handle) {
                ...
                g_am_files_media_handle.media_handle.mp3.aws_plh_init = bt_mp3_aws_plh_init;
                g_am_files_media_handle.media_handle.mp3.aws_plh_deinit = bt_mp3_aws_plh_deinit;
        #endif
                g_prCurrent_player->local_context.files_format.file_event.type = FILE_MP3;
    ...
    #ifdef __AM_DEBUG_INFO__
    bt_sink_srv_report("[Sink][AM]files_codec_open--type: %d, ret: %d, err: %d\n", type, ret, err);
    #endif
    #ifndef MTK_DEBUG_LEVEL_INFO
...
static void am_files_codec_close(void)
{
    ...
    bt_sink_srv_report("[Sink][AM]files_codec_close--type: %d, ret: %d, err: %d, f_state: %d\n", type, ret, err, g_am_file_state);
    #ifndef MTK_DEBUG_LEVEL_INFO
    ...
    uint32_t file_size)
{
    ...
    #ifdef __AM_DEBUG_INFO__
    bt_sink_srv_report("[Sink][AM]skip_id3v2(e)--err: %d, size: %d\n", err, file_size);
    #endif
...
void bt_sink_srv_set_acf_mode(uint8_t mode)
{
    ...
    if (am_speech_eaps) {
        ...
        for (i = 0; i < m; ++i) {
            ...
            bt_sink_srv_report("[Sink][AM]acf--i: %d--0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", i,
    ...
    bt_sink_srv_report("[Sink][AM]acf--ret: %d, mode: %d, k: %d\n", ret, mode, k);
}
#endif
...
extern void bt_sink_srv_local_mode_check(bt_sink_srv_music_mode_t mode);

static int32_t am_set_music_mode(bt_sink_srv_music_mode_t new_mode)
{
    int32_t ret = 0;

    bt_sink_srv_report("[Sink][AM]am_set_mode--old: %d, new: %d\n", g_music_mode, new_mode);
    g_music_mode = new_mode;

    return ret;
}

int32_t bt_sink_srv_ami_check_music_mode(bt_sink_srv_music_mode_t new_mode)
{
    bt_sink_srv_music_mode_t old_mode;
    if (g_music_mode != new_mode) {
        old_mode = g_music_mode;

        // suspend aws if switch mode
        if (old_mode == BT_SINK_SRV_MODE_BT_MUSIC &&
                new_mode != BT_SINK_SRV_MODE_BT_MUSIC) {
#ifdef __MUSIC_MODE_SWITCH_SUPPORT__
            bt_sink_srv_a2dp_mode_check(new_mode);
#endif
        } else if (old_mode == BT_SINK_SRV_MODE_LOCAL_MUSIC &&
                new_mode != BT_SINK_SRV_MODE_LOCAL_MUSIC) {
            bt_sink_srv_local_mode_check(new_mode);
        }
    }
    return 0;
}

int32_t bt_sink_srv_ami_set_music_mode(bt_sink_srv_music_mode_t new_mode)
{
    bt_sink_srv_report("[Sink][AM]set_mode--old: %d, new: %d\n", g_music_mode, new_mode);

    return am_set_music_mode(new_mode);
}

int32_t bt_sink_srv_ami_switch_music_mode(void)
{
    int32_t ret = 0;
#ifdef __BT_AWS_SUPPORT__
    bt_sink_srv_music_mode_t new_mode;
    bt_sink_srv_music_device_t *dev = NULL;

    dev = bt_sink_srv_music_get_device(BT_SINK_SRV_MUSIC_DEVICE_AWS, NULL);
    new_mode = bt_sink_srv_ami_get_next_music_mode();

    if (dev) {
        // it's aws src or mode had not set
        if (dev->aws_role != BT_AWS_ROLE_SINK) {
            bt_sink_srv_ami_check_music_mode(new_mode);
        } else {
            ret = bt_sink_srv_aws_notify_mode_change(dev->aws_hd, BT_AWS_MODE_NONE);
        }
    } else {
        bt_sink_srv_ami_check_music_mode(new_mode);
        ret = am_set_music_mode(new_mode);
        bt_sink_srv_report("[Sink][AM]switch_mode--new: %d\n", new_mode);
    }
    #endif
    return ret;
}

bt_sink_srv_music_mode_t bt_sink_srv_ami_get_next_music_mode(void)
{
    bt_sink_srv_music_mode_t mode = g_music_mode;
    ++mode;
    mode %= BT_SINK_SRV_MODE_END;
    return mode;
}

bt_sink_srv_music_mode_t bt_sink_srv_ami_get_music_mode(void)
{
    return g_music_mode;
}
#endif
