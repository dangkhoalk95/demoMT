...
int16_t bt_sink_srv_audio_tuning_command_entry(char *string)
{
    ...
    if (am_speech_eaps) {
        ...
        bt_sink_srv_report("[Sink][Audio Tunning]command_entry no memory--ply: 0x%x, type: %d\n", g_prCurrent_player, g_prCurrent_player->type);
        ret = -200;
    ...
    if (0 == strncmp(string, "VOL", 3)) {
        if (g_prCurrent_player && (g_prCurrent_player->type == A2DP ||
            ...
            ret = -100;
            bt_sink_srv_report("[Sink][Audio Tunning]error--ply: 0x%x, type: %d\n", g_prCurrent_player, g_prCurrent_player->type);
        }
#if 0
        /* tunning volume */
...
hal_audio_device_t bt_sink_srv_get_tunning_dev(int32_t index, uint8_t type)
{
    ...
    bt_sink_srv_report("[Sink][Audio Tunning]get_dev--dev: 0x%x, index: %d, type: %d\n", dev, index, type);
...
void bt_sink_srv_audio_tunning_update_callback(void *data)
{
    ...
    if (am_speech_eaps) {
        ...
        bt_sink_srv_report("[Sink][Audio Tunning]update_callback no memory--ply: 0x%x, type: %d\n", g_prCurrent_player, g_prCurrent_player->type);
        ret = -200;
    }
    ...
    bt_sink_srv_report("[Sink][Audio Tunning]update_callback--ret: %d\n", ret);    
    #ifndef MTK_DEBUG_LEVEL_INFO
    g_test_fix_warning = ret;
    #endif
}
...
void bt_sink_srv_audio_tunning_update(uint8_t scen, uint8_t type)
{
    ...
    bt_sink_srv_report("[Sink][Audio Tunning]update--ret1: %d, ret2: %d, scen: %d, type: %d\n", ret1, ret2, scen, type);
    #ifndef MTK_DEBUG_LEVEL_INFO
...
void tunning_stream_out_db_transfer(uint16_t gain, uint32_t *digital, uint32_t *analog)
{
    ...
    *analog = (uint32_t)analog_gain_in_db;
    ...
    bt_sink_srv_report("[Sink][Audio Tunning]stream_db--gain: %d, digital: %d, analog: %d\n", gain, digital, analog);
}
...
void tunning_voice_uplink_db_transfer(uint16_t gain, uint32_t *digital, uint32_t *analog)
{
    ...
    *analog = (uint32_t)analog_gain_in_db;
    ...
    bt_sink_srv_report("[Sink][Audio Tunning]voice_db--gain: %d, digital: %d, analog: %d\n", gain, digital, analog);
}
