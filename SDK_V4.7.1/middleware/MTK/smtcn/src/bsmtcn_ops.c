...
static void bsmtcn_start_reset_timer(void);

static void bsmtcn_stop_reset_timer(void);
...
static void aes128_decrypt(uint8_t *p, uint32_t len, uint8_t *key)
{
    ...
    for (i = 0; i < encrypt_data_len / AES_BLK_SIZE; i++) {
        smtcn_aes_decrypt(pCipter + i * AES_BLK_SIZE,
                                       AES_BLK_SIZE,
                                       (uint8_t *)key,
                                       AES_BLK_SIZE,
...
static void bsmtcn_get_info(uint8_t *payload, uint16_t len)
{
    ...
    do{
    	...
    	saved_smtcn_info.ssid_len = p->mix_len.ssid_len;
    	saved_smtcn_info.pwd_len = p->mix_len.password_len;
    	saved_smtcn_info.tlv_data_len = p->mix_len.extend_data_len;
    	...
    	if(saved_smtcn_info.tlv_data_len > BSMTCN_MAX_CUSTOM_LEN) {
    ...
    if(!success)
    {
        saved_smtcn_info.tlv_data_len = 0;
        saved_smtcn_info.pwd_len = 0;
        saved_smtcn_info.ssid_len = 0;
    }
    else
    {
		memcpy(saved_smtcn_info.ssid, &payload[9], saved_smtcn_info.ssid_len);
		printf("ssid: %s\r\n", saved_smtcn_info.ssid);
		bsmtcn_hex_dump("ssid hex \r\n", saved_smtcn_info.ssid, saved_smtcn_info.ssid_len);

		memcpy(saved_smtcn_info.pwd, &payload[9] + saved_smtcn_info.ssid_len, saved_smtcn_info.pwd_len);
		printf("pwd: %s\r\n", saved_smtcn_info.pwd);
		bsmtcn_hex_dump("pwd hex \r\n", saved_smtcn_info.pwd, saved_smtcn_info.pwd_len);

		memcpy(saved_smtcn_info.tlv_data, &payload[9] + saved_smtcn_info.ssid_len + saved_smtcn_info.pwd_len, saved_smtcn_info.tlv_data_len);
...
static void bsmtcn_timeout(TimerHandle_t tmr)
{
    printf("bsmtcn_timeout\r\n");
	bsmtcn_adapter_reset();
	smtcn_continue_switch();
}
...
static int32_t bsmtcn_init(const uint8_t *key, const uint8_t key_length)
{
    ...
    bsmtcn_rst_timer = xTimerCreate("bsmtcn_rst_timer",
                                   (locked_channel_timems / portTICK_PERIOD_MS), 
                                   pdFALSE,
                                   NULL,
                                   bsmtcn_timeout);

    if (bsmtcn_rst_timer == NULL) {
        printf("bsmtcn_rst_timer create fail.\r\n");
        return -1;
    }
...
static void bsmtcn_start_reset_timer(void)
{
    xTimerStart(bsmtcn_rst_timer, tmr_nodelay);
}


static void bsmtcn_stop_reset_timer(void)
{
    xTimerStop(bsmtcn_rst_timer, tmr_nodelay);
}
...
static void bsmtcn_cleanup(void)
{
	...
	if (bsmtcn_rst_timer != NULL) {
        xTimerDelete(bsmtcn_rst_timer, tmr_nodelay);
        bsmtcn_rst_timer = NULL;
    }
...
static int32_t bsmtcn_input(char *p, int32_t len, uint8_t channel)
{
    ...
    */
	...
	if(channel == smtcn_current_channel()) {
...
const smtcn_proto_ops bsmtcn_proto_ops = {
    .init               =   bsmtcn_init,
    .cleanup            =   bsmtcn_cleanup,
    .switch_channel_rst =   bsmtcn_channel_reset,
    .rx_handler         =   bsmtcn_input,
};
