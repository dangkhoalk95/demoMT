...
typedef enum {
    ...
    WIFI_COMMAND_ID_EXTENSION_MAC_RX = 0x2B,                 /**< 0x2B (Set / Query) */
    ...
    WIFI_COMMAND_ID_EXTENSION_STATISTICS = 0x30,             /**< 0x30 (Set / Query) */
    WIFI_COMMAND_ID_EXTENSION_SIGNLE_SKU_5G = 0x31,         /**< 0x31 (Set/Query) */
    WIFI_COMMAND_ID_EXTENSION_AC3_ENABLE = 0x36,            /**< 0x36 (Set/Query) */
    WIFI_COMMAND_ID_EXTENSION_FLOW_CONTROL_THRESHOLD = 0x37, /**< 0x37 (Set/Query) */
    WIFI_COMMAND_ID_EXTENSION_SYS_SLEEP = 0x38               /**< 0x38 (Set/ Query) */
...
/** @brief IP filter for power save mode.
*/
typedef struct {
    uint8_t ip_address[4]; /**< e.g.192.168.0.11-->ip_address[]={192, 168, 0, 11} */
} wifi_ip_filter_t;
...
/**
 * @brief set N9 enter sleep or exit sleep mode  
 *        only 5932 support it 
 *
 * @param [IN] flag_set  1  set the status,  0 get the status
 * @param [IN] onoff  1 means enter sleep, 0 means exit sleep
 *
 *@return  >=0 means success, <0 means fail
 */
int32_t wifi_inband_sys_sleep(uint8_t flag_set, uint8_t *onoff);
...
* @return >=0 means success, <0 means fail
*/
int32_t wifi_inband_get_ps_mode(uint8_t *ps_mode);
...
int32_t wifi_inband_frame_type_filiter(uint8_t filter_switch, uint8_t rx_desc_flag, uint16_t frame_type, extra_vender_ie_t *vender_ie);

int32_t wifi_inband_lmac_rx(uint8_t flag_set, uint8_t *control);

int32_t wifi_inband_get_wlan_statistics(wifi_inband_statistic_t* wifi_inband_statistics);

int32_t wifi_inband_set_ip_filter(wifi_ip_filter_t ip);
...
#endif
int32_t wifi_inband_AC3_enable(uint8_t flag_set, uint8_t  * enable);
int32_t wifi_inband_flow_control_threshold(uint8_t flag_set, uint8_t * low_level,uint8_t * high_level);
