...
static int32_t sbc_codec_event_send_from_isr (sbc_codec_media_handle_t *handle, void *parameter)
{
    ...
    TASK_LOG_I("[sbc_codec_event_send_from_isr] codec queue handle: %x", sbc_codec_queue_handle);
...
{
    TASK_LOG_I("sbc_codec_task_main,code queue handle: %x", sbc_codec_queue_handle);
    sbc_codec_queue_event_t event;
...
sbc_codec_media_handle_t *sbc_codec_open (sbc_codec_callback_t callback, sbc_codec_initial_parameter_t *param, void *user_data)
{
    ...
    if ( internal_handle == NULL ) {
        TASK_LOG_I("internal handle create fail\r\n");
        return 0;
    }
    ...
    switch (channel_mode) {
        ...
        case 1:
            ...
            break;
        default:
            TASK_LOG_I("unknown channel mode %d\r\n", channel_mode);
            vPortFree(internal_handle);
            return 0;
    }
    ...
    switch (sample_rate) {
        ...
        case 1:
            ...
            break;
        default:
            TASK_LOG_I("unknown sample rate %d\r\n", sample_rate);
            vPortFree(internal_handle);
            return 0;
    }
    ...
    if ( sbc_codec_semaphore == NULL ) {
        ...
        TASK_LOG_I("create sbc_semaphore. %0x", sbc_codec_semaphore);
    }

    if(sbc_codec_queue_handle == NULL){
        TASK_LOG_I("create sbc_codec_queue.");
        sbc_codec_queue_handle = xQueueCreate(20, sizeof(sbc_codec_queue_event_t));
        TASK_LOG_I("create sbc_codec_queue. %0x", sbc_codec_queue_handle );
    }

    if(sbc_codec_task_handle == NULL){
        TASK_LOG_I("create sbc_codec_task.");
        xTaskCreate(sbc_codec_task_main,
         "sbc_codec_task", 1024 / sizeof(StackType_t), NULL, 7, &sbc_codec_task_handle);
        TASK_LOG_I("create sbc_codec_task. %0x", sbc_codec_task_handle );
    }
    ...
    if ( sbc_codec_task_handle == NULL || sbc_codec_queue_handle == NULL) {
        TASK_LOG_I("task(%p) or queue(%p) create fail.", sbc_codec_task_handle, sbc_codec_queue_handle);
        return 0;
    }
    ...
    TASK_LOG_I("open sbc codec -- codec queue handle: %x", sbc_codec_queue_handle);
    return handle;
}
