...
void audio_nvdm_update_external_dsp_power_on_default_value(void);

#ifdef MTK_AUDIO_TUNING_ENABLED
typedef struct {
    external_dsp_sending_path_register_info_t register_info[EXTERNAL_DSP_REGISTER_SENDING_PATH_CUSTOMER_REGISTER_AMOUNT];
} external_dsp_power_on_default_value_t;
...
typedef enum {
    DSP_REGISTER_PROFILE_BT,
    DSP_REGISTER_PROFILE_CAR,
    DSP_REGISTER_PROFILE_TUNING,  // in tuning mode
} dsp_register_profile_t;
...
uint16_t audio_nvdm_save_external_dsp_register_value_to_nvdm(void);
