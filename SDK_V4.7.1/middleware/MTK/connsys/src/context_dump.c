...
static int32_t context_dump_intr_save_dump_data(int32_t start_addr, int16_t data_len)
{
    ...
    printf("context_dump_intr_save_dump_data\n");
    ...
    if( do_write_to_ext_flash() ){
        hal_flash_erase_wrap(CRASH_N9_DUMP_EXT_BASE, HAL_FLASH_BLOCK_4K);
        hal_flash_write_wrap(CRASH_N9_DUMP_EXT_BASE, pdump_temp, dump_len);
    }else{
        hal_flash_erase(CRASH_N9_DUMP_BASE, HAL_FLASH_BLOCK_4K);
        hal_flash_write(CRASH_N9_DUMP_BASE, pdump_temp, dump_len);
    }
    if(pdump_temp != NULL) {
        os_free(pdump_temp);
    }
...
int32_t context_dump_intr_receive_process(void)
{
    ...
    pch = (unsigned char *)SHARE_MEM_START_ADDR;
    printf("context_dump_intr_receive_process\n");
