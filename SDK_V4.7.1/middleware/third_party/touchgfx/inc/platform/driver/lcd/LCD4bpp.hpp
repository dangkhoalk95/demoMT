 ...
 *        and rectangles on 4 bits per pixel gray scale displays.
 *
...
/**
 * @fn inline uint8_t LCD4getPixel(const uint8_t* addr, int offset)
 *
 ...
 */
inline uint8_t LCD4getPixel(const uint8_t* addr, int offset)
{
...
/**
 * @fn inline uint8_t LCD4getPixel(const uint16_t* addr, int offset)
 *
 * @brief Get pixel from buffer/image.
 *
 * @param addr   The address.
 ...
 * @return The pixel value.
 */
inline uint8_t LCD4getPixel(const uint16_t* addr, int offset)
{
    return LCD4getPixel(reinterpret_cast<const uint8_t*>(addr), offset);
}

/**
 * @fn inline void LCD4setPixel(uint8_t* addr, int offset, uint8_t value)
 *
 ...
 */
inline void LCD4setPixel(uint8_t* addr, int offset, uint8_t value)
{
    uint8_t data = addr[offset / 2];
#ifdef USE_LSB
...
/**
 * @fn inline void LCD4setPixel(uint16_t* addr, int offset, uint8_t value)
 *
 * @brief Set pixel in buffer.
 *
 * @param [in] addr The address.
 ...
 * @param value     The value.
 */
inline void LCD4setPixel(uint16_t* addr, int offset, uint8_t value)
{
