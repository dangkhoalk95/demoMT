 ...
 *        and rectangles on 2 bits per pixel gray scale displays.
 *
...
/**
 * @fn inline int LCD2shiftVal(int offset)
 *
 ...
 */
inline int LCD2shiftVal(int offset)
{
#ifdef USE_LSB
    return (offset & 3) << 1;
#else
...
/**
 * @fn inline uint8_t LCD2getPixel(const uint8_t* addr, int offset)
 *
 ...
 */
inline uint8_t LCD2getPixel(const uint8_t* addr, int offset)
{
    return (addr[offset >> 2] >> LCD2shiftVal(offset)) & 3;
}

/**
 * @fn inline uint8_t LCD2getPixel(const uint16_t* addr, int offset)
 *
 * @brief Get pixel from buffer/image.
 *
 * @param addr   The address.
 ...
 * @return The pixel value.
 */
inline uint8_t LCD2getPixel(const uint16_t* addr, int offset)
{
    return LCD2getPixel(reinterpret_cast<const uint8_t*>(addr), offset);
}

/**
 * @fn inline void LCD2setPixel(uint8_t* addr, int offset, uint8_t value)
 *
 ...
 */
inline void LCD2setPixel(uint8_t* addr, int offset, uint8_t value)
{
    int shift = LCD2shiftVal(offset);
    addr[offset >> 2] = (addr[offset >> 2] & ~(3 << shift)) | (value << shift);
}

/**
 * @fn inline void LCD2setPixel(uint16_t* addr, int offset, uint8_t value)
 *
 * @brief Set pixel in buffer.
 *
 * @param [in] addr The address.
 ...
 * @param value     The value.
 */
inline void LCD2setPixel(uint16_t* addr, int offset, uint8_t value)
{
