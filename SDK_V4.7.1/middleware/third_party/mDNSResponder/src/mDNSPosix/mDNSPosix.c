...
#include "os_util.h"    // From FreeRTOS 
...
{
    assert(intf != NULL);
    if (intf->intfName != NULL) os_free((void *)intf->intfName);
   ...
   //    }
	 ...
	 if (err ==0 && JoinMulticastGroup)
        {
            if (setsockopt(*sktPtr, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&loopch,  sizeof(loopch)) < 0) {
                lwip_close(*sktPtr);
                debug_printf("setsockopt-IP_MULTICAST_LOOP fail\n");
         }
       }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IP_ADD_MEMBERSHIP"); debug_printf("SetupSocket: setsockopt IP_ADD_MEMBERSHIP err < 0 \n ");}			
        }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IP_MULTICAST_IF");  debug_printf("SetupSocket: setsockopt IP_MULTICAST_IF err < 0 \n ");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IP_TTL");  debug_printf("SetupSocket: setsockopt IP_TTL err < 0 \n ");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IP_MULTICAST_TTL"); debug_printf("SetupSocket: setupsockopt IP_MULTICAST_TTL\n");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("bind"); fflush(stderr); debug_printf("SetupSocket: bind listening socket error \n");}
        }
    ...
    else if (intfAddr->sa_family == AF_INET6)
    {
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_PKTINFO"); debug_printf("SetupSocket: setsockopt - IPV6_PKTINFO error\n");}
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_HOPLIMIT");  debug_printf("SetupSocket: setsockopt - IPV6_HOPLIMIT error\n");}
        }
    #endif
       if (err == 0 && JoinMulticastGroup)
       {
               if (setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (char *)&loopch,  sizeof(loopch)) < 0) {
                   lwip_close(*sktPtr);
                   debug_printf("setsockopt-IPV6_MULTICAST_LOOP fail\n");
                }
       }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
            ...
            {
                err = errno;
                verbosedebugf("IPV6_ADD_MEMBERSHIP %.16a on %d failed.\n", &imr6.ipv6mr_multiaddr, imr6.ipv6mr_interface);
                debug_perror("setsockopt - IPV6_ADD_MEMBERSHIP");
                                 debug_printf("SetupSocket: IPV6_ADD_MEMBERSHIP failed \n");
            }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
           ...
           if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_MULTICAST_IF");  debug_printf("SetupSocket: setsockopt - IPV6_MULTICAST_IF error\n");}
	}
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_V6ONLY");  debug_printf("SetupSocket: setsockopt - IPV6_V6ONLY error\n");}
        }
#endif 
        // Per the mDNS spec, send unicast packets with TTL 255
        if (err == 0)
        {
            err = setsockopt(*sktPtr, IPPROTO_IP, IP_TTL, &kIntTwoFiveFive, sizeof(kIntTwoFiveFive));
            if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_UNICAST_HOPS"); }
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; debug_perror("setsockopt - IPV6_MULTICAST_HOPS");debug_printf("SetupSocket: setsockopt - IPV6_MULTICAST_HOPS error\n"); }
        }
	    ...
	    if (err < 0) { err = errno; debug_perror("bind"); fflush(stderr);debug_printf("SetupSocket: bind listening packet Addr6 error %d \n ", err); }
        }
...
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, const char *intfName, int intfIndex)
{
    ...
    intf_setup = (PosixNetworkInterface*)os_malloc(sizeof(PosixNetworkInterface));
    ...
    if (err == 0)
    {
		...
		intf_setup->intfName = os_malloc(sizeof(intfName));
        // For Keil
...
mDNSlocal int SetupInterfaceList(mDNS *const m)
{
    ...
    intfAddr_temp6 = os_malloc(sizeof(struct sockaddr_in6));
    intfAddr_temp6->sin6_family = AF_INET6;
    os_memcpy(intfAddr_temp6->sin6_addr.un.u32_addr, netif_var->ip6_addr[lwip_ipv6_addr_index].addr, sizeof(netif_var->ip6_addr[lwip_ipv6_addr_index].addr));
    ...
    intfNetmask_temp6 = os_malloc(sizeof(struct sockaddr_in6));
    addrptr = os_malloc(sizeof(struct in6_addr));
    ...
    intfAddr_temp = os_malloc(sizeof(struct sockaddr_in));
    intfAddr_temp->sin_family = AF_INET; 
    intfAddr_temp->sin_addr.s_addr  = netif_var->ip_addr.addr;
    intfAddr = (struct sockaddr*) intfAddr_temp;

    struct sockaddr_in* intfNetmask_temp;
    intfNetmask_temp = os_malloc(sizeof(struct sockaddr_in));
    intfNetmask_temp->sin_family = AF_INET;
    intfNetmask_temp->sin_addr.s_addr  = netif_var->netmask.addr;
...
#if HAVE_IPV6
#if LWIP_IPV6
       os_free(intfAddr_temp6);
       os_free(intfNetmask_temp6);
#endif
#else
       os_free(intfAddr_temp);
       os_free(intfNetmask_temp);
#endif
#endif    
    return err;
}
...
mDNSexport void *  mDNSPlatformMemAllocate(mDNSu32 len) { return(os_malloc(len)); }
mDNSexport void    mDNSPlatformMemFree    (void *mem)   { os_free(mem); }
...
mStatus mDNSPosixAddFDToEventLoop(int fd, mDNSPosixEventCallback callback, void *context)
{
    ...
    newSource = (PosixEventSource*) os_malloc(sizeof *newSource);
    if (NULL == newSource)
        return mStatus_NoMemoryErr;
