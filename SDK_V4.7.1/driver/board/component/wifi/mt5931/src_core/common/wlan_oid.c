...
/*
** $Id: //Department/DaVinci/TRUNK/MT6620_5931_WiFi_Driver/common/wlan_oid.c#93 $
*/
...
#ifdef __WIFI_SNIFFER_SUPPORT__
extern void wndrv_abm_iot_listened_ap_info_ind_handler(kal_uint8 status, kal_uint8 ssid_len, kal_uint8 pwd_len, kal_uint8 *ssid_pwd, kal_uint8 auth_mode);
#endif
...
extern VOID kalSetMediaStateIndicated(IN P_GLUE_INFO_T prGlueInfo,
                                      IN ENUM_PARAM_MEDIA_STATE_T eParamMediaStateIndicate);
extern int sprintf(char *buf, const char *fmt, ...);
extern void wndrv_supc_site_survey_ind_handler(void);
extern void SMS4KeyExt(UINT_8 *Key, UINT_32 *rk, UINT_32 CryptFlag);
extern void SMS4Crypt(UINT_8 *Input, UINT_8 *Output, UINT_32 *rk);
/******************************************************************************
*                              F U N C T I O N S
*******************************************************************************
*/
...
)
{
    ...
    for (u4NumItem = 0; u4NumItem < PARAM_NETWORK_TYPE_NUM ; u4NumItem++) {
        eSupportedNetworks[u4NumItem] = (ENUM_PARAM_NETWORK_TYPE_T)0;
    }
...
#define VER_ERR_MSG     "NVRAM WARNING: Err = 0x%02X"
    if (fgIsConfExist == TRUE) {
        ...
        if (IS_BMCAST_MAC_ADDR(prRegInfo->aucMacAddr) || EQUAL_MAC_ADDR(aucZeroMacAddr, prRegInfo->aucMacAddr)) {
...
)
{
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_SCAN, "wlanoidSetBssidListScan()");

#ifdef __WIFI_SNIFFER_SUPPORT__
    if (prAdapter->u1SnifferOnOff) { //sniffer mode, no allow to scan
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "sniffer mode, wlanoidSetBssidListScanis rejected");
    ...
    *pu4SetInfoLen = 0;
    ...
    if (prAdapter->fgIsRadioOff) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Return from BSSID list scan! (radio off). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_SUCCESS;
    }
...
#if 0    /* NOTE(Nelson): mark to avoid MAUI_03037088 issue */
    scanRemoveBssDescsByPolicy(prAdapter,
                               (SCN_RM_POLICY_EXCLUDE_CONNECTED | SCN_RM_POLICY_ENTIRE));
#endif
...
)
{
...
#ifdef __WIFI_SNIFFER_SUPPORT__
    if (prAdapter->u1SnifferOnOff) { //sniffer mode, no allow to scan
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "sniffer mode, wlanoidSetSsid is rejected");
        wlanClearScanningResult(prAdapter);
        wndrv_supc_site_survey_ind_handler();
        return WLAN_STATUS_SUCCESS;
    }
    ...
    if (u4SetBufferLen < sizeof(PARAM_SSID_T) || u4SetBufferLen > sizeof(PARAM_SSID_T)) {
        ...
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set ssid! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "wlanoidSetSsid:SSID %s\n", prAdapter->rWifiVar.rConnSettings.aucSSID);
...
)
{
    ...
    *pu4SetInfoLen = sizeof(ENUM_PARAM_OP_MODE_T);
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set infrastructure mode! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    if (eOpMode >= NET_TYPE_NUM) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Invalid mode value %d\n", eOpMode);
        return WLAN_STATUS_INVALID_DATA;
    }
    ...
    if (eOpMode == NET_TYPE_IBSS || eOpMode == NET_TYPE_DEDICATED_IBSS) {
        if (cnmAisIbssIsPermitted(prAdapter) == FALSE) {
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Mode value %d unallowed\n", eOpMode);
            return WLAN_STATUS_FAILURE;
        }
    }
...
)
{
    ...
    *pu4SetInfoLen = sizeof(ENUM_PARAM_AUTH_MODE_T);
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set Authentication mode! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    if (*(P_ENUM_PARAM_AUTH_MODE_T)pvSetBuffer >= AUTH_MODE_NUM) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Invalid auth mode %d\n", *(P_ENUM_PARAM_AUTH_MODE_T)pvSetBuffer);
        return WLAN_STATUS_INVALID_DATA;
    }
    ...
    switch (prAdapter->rWifiVar.rConnSettings.eAuthMode) {
        case AUTH_MODE_OPEN:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: open\n");
            break;

        case AUTH_MODE_SHARED:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: shared\n");
            break;

        case AUTH_MODE_AUTO_SWITCH:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: auto-switch\n");
            break;

        case AUTH_MODE_WPA:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: WPA\n");
            break;

        case AUTH_MODE_WPA_PSK:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: WPA PSK\n");
            break;

        case AUTH_MODE_WPA_NONE:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: WPA None\n");
            break;

        case AUTH_MODE_WPA2:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: WPA2\n");
            break;

        case AUTH_MODE_WPA2_PSK:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "New auth mode: WPA2 PSK\n");
            break;

        default:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_WARNING, "New auth mode: unknown (%d)\n", prAdapter->rWifiVar.rConnSettings.eAuthMode);
    }
#endif
...
)
{
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set add key! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    if (prNewKey->u4KeyLength == 5) {
        ...
        if (prAdapter->rWifiVar.rConnSettings.eAuthMode == AUTH_MODE_WPA_NONE) {
            ...
            if (rsnCheckPmkidCandicate(prAdapter)) {
                ...
                MT5931_TRACE0(TRACE_GROUP_10, MT5931_INFO_232, "Add key: Prepare a timer to indicate candidate PMKID Candidate\n");
...
)
{
    ...
    ENUM_PARAM_ENCRYPTION_STATUS_T eEncStatus = (ENUM_PARAM_ENCRYPTION_STATUS_T)0;
...
)
{
    ...
    *pu4SetInfoLen = sizeof(ENUM_PARAM_ENCRYPTION_STATUS_T);
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set encryption status! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }

    eEewEncrypt = *(P_ENUM_PARAM_ENCRYPTION_STATUS_T)pvSetBuffer;
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "ENCRYPTION_STATUS %d\n", eEewEncrypt);
    ...
    switch (eEewEncrypt) {
        ...
        default:
            MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Unacceptible encryption status: %d\n",
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    switch (prTest->u4Type) {
        case 1:     /* Type 1: generate an authentication event */
            ...
            pvStatusBuffer = (PVOID)ALIGN_4((UINT_32)&prAdapter->aucIndicationEventBuffer[0]);
    ...
    for (i = 0; i < prPmkid->u4BSSIDInfoCount; i++) {
        ...
        if (j < CFG_MAX_PMKID_CACHE) {
            kalMemCopy(prAisSpecBssInfo->arPmkidCache[j].rBssidInfo.arPMKID,
                       ...
                       sizeof(PARAM_PMKID_VALUE));
            MT5931_TRACE(TRACE_GROUP_10, MT5931_INFO_233, "Add BSSID "MACSTR" idx=%d PMKID value "MACSTR"\n",
...
)
{
    ...
    if (u4SetBufferLen < sizeof(PARAM_RATES)) {
        DBGLOG(REQ, WARN, ("Invalid length %d\n", u4SetBufferLen));
        return WLAN_STATUS_INVALID_LENGTH;
    }
...
)
{
    ...
    *pu4QueryInfoLen = sizeof(PARAM_RSSI);
    ...
    return wlanSendSetQueryCmd(prAdapter,
                               ...
                               TRUE,
                               TRUE,
                               NULL, //nicCmdEventQueryLinkQuality,
                               nicOidCmdTimeoutCommon,
                               *pu4QueryInfoLen,
                               pvQueryBuffer,
                               pvQueryBuffer,
    ...
    return wlanSendSetQueryCmd(prAdapter,
                               ...
                               TRUE,
                               TRUE,
                               //nicCmdEventQueryLinkQuality,    /* NOTE(Nelson): nicCmdEventQueryLinkQuality() is not needed */
                               NULL,
                               nicOidCmdTimeoutCommon,
                               0,
                               NULL,
    ...
    if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) != PARAM_MEDIA_STATE_CONNECTED) {
        ...
        *(PUINT_32)pvQueryBuffer = prAdapter->rLinkQuality.u2LinkSpeed * 5000; // change to unit of 100bps
        ...
        return wlanSendSetQueryCmd(prAdapter,
                                   ...
                                   TRUE,
                                   TRUE,
                                   //nicCmdEventQueryLinkSpeed,    /* NOTE(Nelson): nicCmdEventQueryLinkSpeed() is not needed */
                                   NULL,
                                   nicOidCmdTimeoutCommon,
                                   0,
                                   NULL,
    ...
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUC_T);
    ...
    switch (u2Id) {
        ...
        case 0xFFFF: {
            ...
            } else if (u2SubId == 0x1234) { // NOTE(Nelson): CTIA mode checked
...
)
{
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_DISCONN, "wlanoidSetDisassociate");
    ...
    *pu4SetInfoLen = 0;
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Fail in set disassociate! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                         prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
...
)
{
    WLAN_STATUS status;
    PARAM_POWER_MODE ePowerMode;
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_INFO, "wlanoidSet802dot11PowerSaveProfile,MODE=%d", *(PPARAM_POWER_MODE) pvSetBuffer);
    ...
    *pu4SetInfoLen = sizeof(PARAM_POWER_MODE);
    ...
    switch (ePowerMode) {
        case Param_PowerModeCAM:
            ...
            MT5931_TRACE(TRACE_GROUP_8, MT5931_INFO_234, "Set Wi-Fi PS mode to CAM (%d)\n", ePowerMode);
            break;
        case Param_PowerModeMAX_PSP:
            //DBGLOG(INIT, INFO, ("Set Wi-Fi PS mode to MAX PS (%d)\n", ePowerMode));
            MT5931_TRACE(TRACE_GROUP_8, MT5931_INFO_235, "Set Wi-Fi PS mode to MAX PS (%d)\n", ePowerMode);
            break;
        case Param_PowerModeFast_PSP:
            //DBGLOG(INIT, INFO, ("Set Wi-Fi PS mode to FAST PS (%d)\n", ePowerMode));
            MT5931_TRACE(TRACE_GROUP_8, MT5931_INFO_236, "Set Wi-Fi PS mode to FAST PS (%d)\n", ePowerMode);
            break;
        default:
            //DBGLOG(INIT, INFO, ("invalid Wi-Fi PS mode setting (%d)\n", ePowerMode));
            MT5931_TRACE(TRACE_GROUP_8, MT5931_INFO_237, "invalid Wi-Fi PS mode setting (%d)\n", ePowerMode);
            break;
    }
...
)
{
    ...
    *pu4SetInfoLen = sizeof(UINT_32);
    ...
    if ((*pu4BeaconInterval < DOT11_BEACON_PERIOD_MIN) ||
        ...
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "Invalid Beacon Interval = %ld\n", *pu4BeaconInterval);
        return WLAN_STATUS_INVALID_DATA;
    }

    prAdapter->rWlanInfo.u2BeaconPeriod = (UINT_16) * pu4BeaconInterval;

    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "Set beacon interval: %d\n",
    ...
    if (prAdapter->fgEnArpFilter) {
        ...
        for (i = 0, j = 0 ; i < prNetworkAddressList->u4AddressCount ; i++) {
            if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
                ...
                MT5931_TRACE(TRACE_GROUP_8, MT5931_INFO_238, "prNetAddrIp->in_addr:%d:%d:%d:%d\n", pucBuf[0], pucBuf[1], pucBuf[2], pucBuf[3]);
            }
...
)
{
    ...
    //return WLAN_STATUS_PENDING;
    return WLAN_STATUS_SUCCESS; //NOTE(Nelson): workaround for META mode
}
...
)
{
    ...
    // increase command sequence number
    ucCmdSeqNum = nicIncreaseCmdSeqNum(prAdapter);
    // DBGLOG(REQ, TRACE, ("ucCmdSeqNum =%d / CmdID=0x%x\n", ucCmdSeqNum, ucCID));
    //MT5931_TRACE(TRACE_GROUP_6, MT5931_INFO_239, "ucCmdSeqNum =%d / CmdID=%x\n", ucCmdSeqNum, ucCID);
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called by x to adjuge if WAPI support.
*
* \param[in] null
*
* \retval 1: WAPI suppot.
* \retval 0: WAPI not support.
*
*/
/*----------------------------------------------------------------------------*/
WLAN_STATUS
...
)
{
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    if (prNewKey->ucKeyID != 0x1 ||
        ...
        //DBGLOG(SEC, ERROR, ("Invalid WAPI key ID (%d)\r\n", prNewKey->ucKeyID));
    }
    ...
    if (prNewKey->eKeyType == ENUM_WPI_PAIRWISE_KEY) {
        prCmdKey->ucTxKey = 1;
        prCmdKey->ucKeyType = 1;
        /* NOTE(Nelson): workaround for supc not carry peer's bssid */
        kalMemCopy(prCmdKey->aucPeerAddr, prAdapter->rWlanInfo.rCurrBssId.arMacAddress, MAC_ADDR_LEN);
    } else {
        /* NOTE(Nelson): workaround for supc not carry peer's bssid */
        kalMemSet(prCmdKey->aucPeerAddr, 0xff, MAC_ADDR_LEN);
    }

#if 0   /* NOTE(Nelson): workaround for supc not carry peer's bssid */
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to query WAPI capability
*
* \param[in] prAdapter          Pointer to the Adapter structure.
* \param[in] pvQueryBuffer      Pointer to the buffer that holds the result of
*                               the query.
...
{
    PUINT_32                    pu4Value;
    UINT_32                     prmackey_in[32];
    UINT_32                     input[4] = {0x1351757A, 0x2FAF7465, 0x7BEBC9D, 0x2CA0B2E4};

    DEBUGFUNC("wlanoidQueryWapiCap");

    WNDRV_ASSERT(prAdapter);
    WNDRV_ASSERT(pu4QueryInfoLen);
    if (u4QueryBufferLen) {
        WNDRV_ASSERT(pvQueryBuffer);
    }

    *pu4QueryInfoLen = sizeof(UINT_32) * 8;
    ...
    pu4Value = (PUINT_32)pvQueryBuffer;

    SMS4KeyExt((PUINT_8)input, (PUINT_32)prmackey_in, 0);
    SMS4Crypt((PUINT_8)&pu4Value[0], (PUINT_8)&pu4Value[4], (PUINT_32)prmackey_in);

    //wpi_get_cap(&pu4Value[0], &pu4Value[4]);
...
)
{
    ...
    *pu4SetInfoLen = 2;
    ...
    if (TRUE == fgIsForceReSearchCountryCode) {
...
)
{
    ...
    DEBUGFUNC("wlanoidSetPreferredAnt");

    WNDRV_ASSERT(prAdapter);
    WNDRV_ASSERT(pvSetBuffer);
    WNDRV_ASSERT(pu4SetInfoLen);
    ...
    switch (u4Value) {
        case 0:/* WiFi */
            ...
            //status = wlanoidSetMcrWrite(prAdapter, &rMcrWrInfo, sizeof(rMcrWrInfo), &u4InfoLen);
            kalUdelay(500);
            /* Switch ANT path */
#if 1
...
#endif
            prAdapter->fgIsAntBT = TRUE;
            break;
        default:
            //RETAILMSG(TRUE, (L"Unexpected antenna %d\n", u4Value));
            return WLAN_STATUS_INVALID_DATA;
    }
...
)
{
    ...
    //DEBUGFUNC("wlanoidQueryPreferredAnt");
    WNDRV_ASSERT(prAdapter);
    WNDRV_ASSERT(pu4QueryInfoLen);
    if (u4QueryBufferLen) {
        WNDRV_ASSERT(pvQueryBuffer);
...
#ifdef __WIFI_SNIFFER_SUPPORT__
WLAN_STATUS wlanoidSetSnifferMode(
    IN P_ADAPTER_T  prAdapter,
    IN  PVOID    pvSetBuffer,
    IN  UINT_32  u4SetBufferLen,
    OUT PUINT_32 pu4SetInfoLen
...
{
    P_PARAM_CUSTOM_SET_SNIFFER_MODE_T prSnifferInfo;
    CMD_SET_SNIFFER_MODE_T rSnifferInfo;
    UINT_32 u4NewPacketFilter;
    WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_SET_SNIFFER_MODE_T);


    DEBUGFUNC("wlanoidSetSnifferMode");

    if (u4SetBufferLen < sizeof(PARAM_CUSTOM_SET_SNIFFER_MODE_T)) {
        return WLAN_STATUS_INVALID_LENGTH;
    }


    prSnifferInfo = (P_PARAM_CUSTOM_SET_SNIFFER_MODE_T) pvSetBuffer;

    u4NewPacketFilter = (UINT_32)(prSnifferInfo->ucSnifferFilterOptions & PARAM_PACKET_FILTER_SNIFFER_MASK);

    if (rStatus == WLAN_STATUS_SUCCESS) {

        if ((prSnifferInfo->ucOnOffSwitch == PARA_SNIFFER_MODE_ON_WITH_PARA) && (u4NewPacketFilter)) { //sniffer mode

            // Store the packet filter
            if (!prAdapter->u4CurrentPacketFilter) {
                prAdapter->u4CurrentPacketFilter = prAdapter->u4OsPacketFilter;
            }
            prAdapter->u4OsPacketFilter |= u4NewPacketFilter;//store packet filter and sniffer filter

            prAdapter->smart_config.u1ListenState = 0;

            prAdapter->u1SnifferOnOff = (UINT_8)prSnifferInfo->ucOnOffSwitch;

            if (prSnifferInfo->ucChannelNum < 15 && prSnifferInfo->ucChannelNum > 0) {
                rSnifferInfo.ucOnOffSwitch = 1;
                rSnifferInfo.u2MaxPacketLength = prSnifferInfo->u2MaxPacketLength;
                rSnifferInfo.u2MinPacketLength = prSnifferInfo->u2MinPacketLength;
                rSnifferInfo.ucChannelNum = prSnifferInfo->ucChannelNum;
                rSnifferInfo.ucSnifferFilterOptions = u4NewPacketFilter;

                //20<rcpimin<=rcpimax<220,enable rssi filter
                if ((prSnifferInfo->cMaxRssi < 0) && (prSnifferInfo->cMinRssi > -100) && (prSnifferInfo->cMinRssi <= prSnifferInfo->cMaxRssi)) {
                    rSnifferInfo.cMaxRssi = dBm_TO_RCPI(prSnifferInfo->cMaxRssi);
                    rSnifferInfo.cMinRssi = dBm_TO_RCPI(prSnifferInfo->cMinRssi);
                } else { //disable rssi filter option
                    rSnifferInfo.cMaxRssi = 10;
                    rSnifferInfo.cMinRssi = 10;
                }

                MT5931_NEW_TRACE(MT5931_INFO_608, TRACE_INFO, "sniffer:OnOff(%d),Channel(%d),Filter(0x%x),MinRcpi(%d),MaxRssi(%d),MinLen(%d),MaxLen(%d)", rSnifferInfo.ucOnOffSwitch, \
            ...
            return WLAN_STATUS_SUCCESS;
        } else if (prSnifferInfo->ucOnOffSwitch == PARA_SNIFFER_MODE_ON_WITHOUT_PARA) { //smart connection
            status_cb ap_info_func = (status_cb) wndrv_abm_iot_listened_ap_info_ind_handler;
            kal_uint8 channeltable[19] = {1, 6, 11, 2, 5, 7, 1, 6, 11, 10, 12, 3 , 1, 6, 11, 8, 13, 4, 9};
            UINT_32 u4SmartConfigFilter = 0x00100000;//muticast

            kalMemCopy(&prAdapter->smart_config.u1ListenChannelTable[0], &channeltable[0], 19);
            if (prAdapter->u1SnifferOnOff == (UINT_8)PARA_SNIFFER_MODE_ON_WITHOUT_PARA) {
                return WLAN_STATUS_SUCCESS;
            }
            ...
            prAdapter->u4OsPacketFilter |= u4SmartConfigFilter;//store packet filter and sniffer filter

            prAdapter->u1SnifferOnOff = (UINT_8)prSnifferInfo->ucOnOffSwitch;
            prAdapter->smart_config.u1ListenState = 1;
            prAdapter->smart_config.u1ListenChannelIndex = 0;

            //1. init
            parser_fsm_init((char *)prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX].aucOwnMacAddr, ap_info_func);
            ...
            rSnifferInfo.ucSnifferFilterOptions = u4SmartConfigFilter;//muticast
            rSnifferInfo.cMinRssi = 25;
            rSnifferInfo.cMaxRssi = 215;

            MT5931_NEW_TRACE(MT5931_INFO_609, TRACE_INFO, "start smart_connect:OnOff(%d),Channel(%d),Filter(0x%x),MinRcpi(%d),MaxRssi(%d),MinLen(%d),MaxLen(%d)\n", rSnifferInfo.ucOnOffSwitch, \
                             rSnifferInfo.ucChannelNum, rSnifferInfo.ucSnifferFilterOptions, rSnifferInfo.cMinRssi, rSnifferInfo.cMaxRssi, \
                             rSnifferInfo.u2MinPacketLength, rSnifferInfo.u2MaxPacketLength);
            ...
            wlanSendSetQueryCmd(prAdapter,
                                ...
                                nicOidCmdTimeoutCommon,
                                sizeof(UINT_32),
                                (PUINT_8)&rSnifferInfo.ucSnifferFilterOptions,
                                pvSetBuffer,
                                u4SetBufferLen
                               );
            //3.start timer
            kal_event_start_timer(&prAdapter->prGlueInfo->prParamWndrvEs[PARAM_WNDRV_ES_SMART_CONFIG_TIMER], 4 * KAL_TICKS_50_MSEC);
            return WLAN_STATUS_SUCCESS;
        } else { //disable

            MT5931_NEW_TRACE0(MT5931_INFO_611, TRACE_INFO, "disable sniffer mode");

            prAdapter->u4OsPacketFilter = prAdapter->u4CurrentPacketFilter;//resume packet filter
            prAdapter->u4CurrentPacketFilter = 0;
