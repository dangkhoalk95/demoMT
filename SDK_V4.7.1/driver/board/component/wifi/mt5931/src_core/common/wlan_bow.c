...
/*
** $Id: //Department/DaVinci/TRUNK/MT6620_5931_WiFi_Driver/common/wlan_bow.c#38 $
*/
...
)
{
    ...
    UINT_8 ucNumOfChannel = MAX_BOW_NUMBER_OF_CHANNEL;
...
#endif
        rlmDomainGetChnlList(prAdapter, BAND_2G4, &ucNumOfChannel, aucChannelList);
...
#if CFG_BOW_TEST
    DBGLOG(BOW, EVENT, ("ucNumOfChannel, eBand, aucChannelList, %x, %x, %x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x.\n",
                        ucNumOfChannel,
                        aucChannelList[5].eBand,
    ...
    DBGLOG(BOW, EVENT, ("prMacStatus->ucNumOfChannel, eBand, prMacStatus->arChannelList, %x, %x, %x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x.\n",
                        prMacStatus->ucNumOfChannel,
                        prMacStatus->arChannelList[5].ucChannelBand,
...
)
{
    ...
    rCmdBtOverWifi.ucChannelNum = prBowSetupConnection->ucChannelNum;           //Fixed to 2.4G
    ...
    rCmdBtOverWifi.ucChannelBand = BAND_2G4;        //Fixed to 2.4G
