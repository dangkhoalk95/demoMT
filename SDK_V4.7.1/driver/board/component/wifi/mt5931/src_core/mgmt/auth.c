...
/*
** $Id: //Department/DaVinci/TRUNK/MT6620_5931_WiFi_Driver/mgmt/auth.c#13 $
*/
...
)
{
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "Send Auth Frame\n");
...
)
{
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_CONN, "Send Auth Frame %d, Status Code = %d\n",
    ...
    /* Allocate a MSDU_INFO_T */
    if ((prMsduInfo = cnmMgtPktAlloc(prAdapter, u2EstimatedFrameLen)) == NULL) {
        MT5931_WAP_TRACE(MOD_WNDRV, TRACE_ERROR, "No PKT_INFO_T for sending Auth Frame.\n");
        return WLAN_STATUS_RESOURCES;
    }
    ...
    /* record TxSeq for hostap loopback */
    prStaRec->ucAuthTxSeqNum = prMsduInfo->ucTxSeqNum;
...
)
{
    ...
    MT5931_WAP_TRACE(MOD_WNDRV, TRACE_DISCONN, "Send Deauth Frame \n");
    if (prClassErrSwRfb) {
        ...
        for (i = NETWORK_TYPE_AIS_INDEX; i < NETWORK_TYPE_INDEX_NUM; i++) {
            ...
            if (IS_NET_ACTIVE(prAdapter, i) &&
                    (EQUAL_MAC_ADDR((void *)prWlanMacHeader->aucAddr1, (void *)prBssInfo->aucOwnMacAddr))) {
                {
    ...
    if (i4NewEntryIndex > 0) {
        ...
        MT5931_TRACE0(TRACE_WARNING, MT5931_INFO_32, "No unused DEAUTH_INFO_T !");
    }
    ...
    if ((prMsduInfo = cnmMgtPktAlloc(prAdapter, u2EstimatedFrameLen)) == NULL) {
        ...
        MT5931_TRACE0(TRACE_WARNING, MT5931_INFO_33, "No PKT_INFO_T for sending Deauth Request.");
        return WLAN_STATUS_RESOURCES;
    }
    ...
    if (prClassErrSwRfb) {
        ...
        eNetTypeIndex = (ENUM_NETWORK_TYPE_INDEX_T)prStaRec->ucNetTypeIndex;
...
)
{
    ...
    if (UNEQUAL_MAC_ADDR((void *)prDeauthFrame->aucBSSID, (void *)aucBSSID)) {
        //DBGLOG(SAA, LOUD, ("Ignore Deauth Frame from other BSS ["MACSTR"]\n",
        //    MAC2STR(prDeauthFrame->aucSrcAddr)));
        MT5931_TRACE(TRACE_WARNING, MT5931_INFO_34, "Ignore Deauth Frame from other BSS ["MACSTR"]",
                     MAC2STR(prDeauthFrame->aucSrcAddr));
        return WLAN_STATUS_FAILURE;
    }
...
)
{
    ...
    //4 <2> Check the BSSID
    if (UNEQUAL_MAC_ADDR((void *)prAuthFrame->aucBSSID, (void *)aucExpectedBSSID)) {
