...
TimerHandle_t lcd_watchdog_timer = NULL;
#endif
...
extern LCD_Funcs LCD_func_RM69080;
extern LCD_Funcs LCD_func_RM67160;
...
{
#ifdef FREERTOS_ENABLE
    xTimerStopFromISR(lcd_watchdog_timer, 0);
#endif
...
#else
    log_hal_info("watch dog timeout\n");
#endif
    bsp_lcd_callback();
}
...
bsp_lcd_status_t bsp_lcd_init(uint16_t bgcolor)
{
    ...
    lcd_watchdog_timer = xTimerCreate("LCD_WTD", (3 * 1000 / portTICK_PERIOD_MS), pdFALSE, NULL, (TimerCallbackFunction_t)bsp_lcd_watchdog);
#endif
...
bsp_lcd_status_t bsp_lcd_update_screen(uint32_t start_x,  uint32_t start_y, uint32_t end_x, uint32_t end_y)
{
    ...
    xTimerStart(lcd_watchdog_timer, 0);
#endif
