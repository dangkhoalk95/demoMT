...
{
    struct clock_src_sel *target = NULL;
#ifdef CLK_DEBUG
    log_hal_info("%s: clock_source_sel_id = %d\n", __FUNCTION__, clock_source_sel_id);
#endif
    if (clock_source_sel_id < NR_CLKSRC_SEL) {
        target = &clock_src_sels[clock_source_sel_id];
...
ATTR_TEXT_IN_TCM uint32_t clock_get_freq_meter(uint16_t tcksel, uint16_t fcksel, bool over78m)
{
    ...
    if (over78m) {
        ...
        target_freq_64 = MIXEDSYSD->ABIST_FQMTR_DATA;
...
ATTR_TEXT_IN_TCM void cm_bus_clk_26m_lfosc_lfosc(void)
{
    ...
    if (clock_dfs_initialized == false) {
        ...
        /* CONFIGSYS->CLK_CONDB &= (~(0x1 << 31)); */ /* Do not turn off PLL_DIV_EN */
        CONFIGSYS->CLK_SOURCE_SEL = 0x00444444;
    } else {
...
ATTR_TEXT_IN_TCM void cm_bus_clk_26m_dcxo_dcxo(void)
{
    ...
    if (clock_dfs_initialized == false) {
        ...
        CONFIGSYS->CLK_CONDD = 0x10000000;
        CONFIGSYS->CLK_CONDA = (0xB00 | 0x0);
        /* CONFIGSYS->CLK_CONDB &= (~((uint32_t) 0x1 << 31)); */ /* Do not turn off PLL_DIV_EN */
        CONFIGSYS->CLK_SOURCE_SEL = 0x00111111;
    } else {
...
ATTR_TEXT_IN_TCM static hal_clock_status_t general_clock_resume_op(void)
{
    ...
    /* restore the pll request */
    if (pll_clock[SYSCLK].saved_pll_req_state) {
        pll_clock[SYSCLK].ops->restore(&pll_clock[SYSCLK]);
        pll_stable_wait++;
    }
    if (pll_clock[UPLLCLK].saved_pll_req_state) {
        pll_clock[UPLLCLK].ops->restore(&pll_clock[UPLLCLK]);
        pll_stable_wait++;
    }
    if (pll_clock[MPLLCLK].saved_pll_req_state) {
        pll_clock[MPLLCLK].ops->restore(&pll_clock[MPLLCLK]);
        pll_stable_wait++;
    }

    if (pll_stable_wait > 0) {
        hal_gpt_delay_us(100);
    }
...
#ifdef CLK_CALI_LFOSC_TEST
ATTR_TEXT_IN_TCM uint32_t clock_lfosc_cali_up()
{
    uint32_t tmp = 0, tmp_cali = 0, tmp_ft = 0;
    uint32_t while_count = 0, k_init = 0, k_before = 249600, k_after = 0;
    uint32_t upper_bound = 0, lower_bound = 0;
    uint32_t magic_before = 0, magic_after = 0, desired_fqdata = 0;
    uint32_t start = 0, end = 0, sum = 0;

    while (1) {
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &start);
        MIXEDSYSD->ABIST_FQMTR_CON2 = 0x0000;
        MIXEDSYSD->ABIST_FQMTR_CON1 = 0x020B;
        MIXEDSYSD->ABIST_FQMTR_CON0 = 0xCFFF;
        MIXEDSYSD->ABIST_FQMTR_CON0 = 0x8FFF;
        tmp = MIXEDSYSD->ABIST_FQMTR_CON1;
        tmp = MIXEDSYSD->ABIST_FQMTR_CON1;
        while (tmp & (0x1 << 15)) {
            tmp = MIXEDSYSD->ABIST_FQMTR_CON1;
        }
        tmp = MIXEDSYSD->ABIST_FQMTR_DATA;
        ...
        if (while_count == 0) {
            /* initial setting */
            k_init = k_after;
            upper_bound = k_after * 1.025;
            lower_bound = k_after * 1.024;
            desired_fqdata = upper_bound / 26 / 1 * 4096 / 10000;
            tmp_cali = (MIXEDSYSD->LFOSC_CON0 & 0x3F00) >> 8;
            tmp_ft = (MIXEDSYSD->LFOSC_CON0 & 0xF0) >> 4;
            magic_before = tmp_cali * tmp + 10 * tmp;
            if (magic_before - (10 * tmp) < 0) {
                magic_before = 10 * tmp - tmp_cali;
            }
            magic_after = magic_before;
        }

        if ((k_after >= lower_bound) && (k_after <= upper_bound) || (while_count >= 50)) {
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &end);
            sum = sum + end - start;
            log_hal_info("[LFOSC] Before K = %d, After K = %d\n\r", k_init, k_after);
            if (while_count >= 50) {
                log_hal_info("[LFOSC] calibration failed\n\r");
            }
            log_hal_info("[%d] calibration cost %d us, tmp = %d, magic_before = %d, magic_after = %d\n\r", while_count, end - start, tmp, magic_before, magic_after);
            break;
        ...
        MIXEDSYSD->LFOSC_CON0 = (MIXEDSYSD->LFOSC_CON0 & 0xC0FF) | (tmp_cali << 8);
        MIXEDSYSD->LFOSC_CON0 = (MIXEDSYSD->LFOSC_CON0 & 0xFF0F) | (tmp_ft << 4);

        hal_gpt_delay_us(6);
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &end);
        sum = sum + end - start;

        log_hal_info("[%d] calibration cost %d us, cali = %d, ft = %d, k_before = %d, k_after = %d, magic_before = %d, magic_after = %d, tmp = %d\n\r", while_count, end - start, tmp_cali, tmp_ft, k_before, k_after, magic_before, magic_after, tmp);
...
ATTR_TEXT_IN_TCM uint32_t clock_lfosc_cali_down()
{
    uint32_t tmp = 0, tmp_cali = 0, tmp_ft = 0;
    uint32_t while_count = 0, k_init = 0, k_before = 249600, k_after = 0;
    uint32_t upper_bound = 0, lower_bound = 0;
    ...
    while (1) {
        ...
        if (while_count == 0) {
            /* initial setting */
            k_init = k_after;
            upper_bound = k_after * 0.976;
            lower_bound = k_after * 0.975;
            desired_fqdata = upper_bound / 26 / 1 * 4096 / 10000;
            tmp_cali = (MIXEDSYSD->LFOSC_CON0 & 0x3F00) >> 8;
            tmp_ft = (MIXEDSYSD->LFOSC_CON0 & 0xF0) >> 4;
            magic_before = tmp_cali * tmp + 10 * tmp;
...
ATTR_TEXT_IN_TCM void clock_dump_log(void)
{
    ...
    uint32_t reg_tmp0, reg_tmp1, reg_tmp2, reg_tmp3, reg_tmp4, reg_tmp5, reg_tmp6, reg_tmp7;
#if 0
    uint32_t reg_tmp8;
#endif
    ...
    /* This part should be moved out into FHP driver */
#if 0
...
ATTR_TEXT_IN_TCM void clock_dump_info(void)
{
    ...
    log_hal_info("%s: CLK_USE_%s, cm4_freq=%d, bus_freq=%d, lfosc=%d, efuse_data=0x%x, peri_bus=%d\n",
                 __FUNCTION__, (is_clk_use_lfosc() ? "LFOSC" : "DCXO"),
                 freq_monitor(19, true), freq_monitor(8, false),
