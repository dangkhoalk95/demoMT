...
extern hal_gpio_pin_t g_uart_port_to_txpin[];
extern uint8_t g_uart_port_to_txpinmux[];
extern uint8_t g_uart_port_to_gpio;
#endif
...
void uart_backup_all_registers(void)
{
    ...
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        if (g_uart_hwstatus[uart_port] != UART_HWSTATUS_UNINITIALIZED) {
            ...
            hal_pinmux_set_function(g_uart_port_to_txpin[uart_port], g_uart_port_to_gpio);
            hal_gpio_set_direction(g_uart_port_to_txpin[uart_port], HAL_GPIO_DIRECTION_OUTPUT);
            hal_gpio_set_output(g_uart_port_to_txpin[uart_port], HAL_GPIO_DATA_HIGH);
...
void uart_restore_all_registers(void)
{
    ...
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        ...
        if (uart_hwstatus != UART_HWSTATUS_UNINITIALIZED) {
            ...
            hal_pinmux_set_function(g_uart_port_to_txpin[uart_port], g_uart_port_to_txpinmux[uart_port]);
...
hal_uart_status_t hal_uart_init(hal_uart_port_t uart_port, hal_uart_config_t *uart_config)
{
    ...
    if (g_uart_restore_init == false) {
        ...
        /* workaround for 25x3 uart tx drop to low when enter sleep */
        uart_pin_init_config(uart_port);
    }
#endif
