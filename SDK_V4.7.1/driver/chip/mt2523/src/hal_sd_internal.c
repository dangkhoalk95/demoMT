...
sd_internal_status_t sd_query_switch_high_speed(hal_sd_port_t sd_port, uint32_t argument)
{
    ...
    if (((argument & 0x01) == switch_status.group1_result) && (0 == (switch_status.group1_status & (1 << 1)))) {
...
sd_internal_status_t sd_deselect_high_speed(hal_sd_port_t sd_port)
{
    sd_internal_status_t status;
    uint32_t i = 1000;
    ...
    do {
        status = sd_query_switch_high_speed(sd_port, SD_COMMAND6_QUERY_HIGH_SPEED);
        if (NO_ERROR == status) {
            status = sd_query_switch_high_speed(sd_port, SD_COMMAND6_DESELECT_HIGH_SPEED);
            if (NO_ERROR == status) {
...
void sd_set_output_clock(hal_sd_port_t sd_port, uint32_t clock)
{
    sd_internal_status_t status;
    ...
    if (sd_information.card_type == HAL_SD_TYPE_SD_CARD       ||
        ...
        if ((sd_information.scr.spec_ver > SD_SPECIFICATION_101) &&
                (sd_information.is_high_speed == MSDC_FALSE) &&
                (clock > SD_DEFAULT_SPEED_MAX)) {
            sd_set_block_length(sd_port, COMMAND6_RESPONSE_LENGTH);
            status = sd_select_high_speed(sd_port);
            if (NO_ERROR != status) {
                /*if card not support high speed, the max speed is 25MHZ*/
                sd_information.is_high_speed = MSDC_FALSE;
                log_hal_error("select sd high speed mode fail.\r\n");
            }
        } else if ((sd_information.is_high_speed == MSDC_TRUE) &&
                   (clock <= SD_DEFAULT_SPEED_MAX)) {
            sd_set_block_length(sd_port, COMMAND6_RESPONSE_LENGTH);
            status = sd_deselect_high_speed(sd_port);
            if (NO_ERROR != status) {
                log_hal_error("deselect sd high speed mode fail, status = %d.\r\n", status);
            }
        }
    }
    ...
    status = sd_set_block_length(sd_port, SD_BLOCK_LENGTH);
    if (NO_ERROR != status) {
        log_hal_error("set block length error, status = %d.\r\n", status);
    }
}
...
bool sd_output_clock_tuning(hal_sd_port_t sd_port, uint32_t clock)
{
    ...
    if (PMIC_VOL_0P9V == vcore_voltage) {
        while (i < 3) {
            if (0 == i) {
                sd_set_output_clock(sd_port, msdc_output_clock[6].output_clock);
            } else {
                sd_set_output_clock(sd_port, msdc_output_clock[6].output_clock / (i * 2));
            }
        ...
        return MSDC_FALSE;
    } else if ((PMIC_VOL_1P1V <= vcore_voltage) && (PMIC_VOL_1P3V > vcore_voltage) && (SystemCoreClock > CLK_CM4_FREQ_26M)) {
        while (i < 5) {
            sd_set_output_clock(sd_port, msdc_output_clock[2 + i].output_clock);
            if (sd_latch_command_test(sd_port) && sd_latch_data_test(sd_port)) {
#ifdef HAL_DVFS_MODULE_ENABLED
        ...
        while (i < 7) {
            sd_set_output_clock(sd_port, msdc_output_clock[0 + i].output_clock);
            if (sd_latch_command_test(sd_port) && sd_latch_data_test(sd_port)) {
                goto end;
            }
            i++;
    ...
    if (msdc_get_output_clock((msdc_port_t)sd_port) > clock) {
        sd_set_output_clock(sd_port, clock);
    }
...
void sd_restore_all(void)
{
    ...
    sd_restore_all_register(sd_backup_parameter.sd_port);//bruce.lee 20180124
