 ...
 **********************************************************************/
static int32_t MountDevice(NOR_FLASH_DISK_Data *D)
{
    int32_t Result;
    ...
    if (Result < FS_NO_ERROR) {
        //it's ok as it doesn't support FS
        //ASSERT(0);
    }
...
hal_flash_status_t get_rawdisk_error_code(int32_t ori_err_code)
{
    if (ori_err_code == RAW_DISK_ERR_WRONG_ADDRESS) {
        ...
        return HAL_FLASH_STATUS_ERROR_ERASE_FAIL;
    } else if (ori_err_code == RAW_DISK_ERR_FDM_LOCKED || ori_err_code == RAW_DISK_ERR_DRV_LOCKED) {
...
int32_t OTP_Write_Data(NOR_FLASH_DISK_Data *D, uint32_t otp_addr, uint8_t *data_ptr, uint32_t len)
{
    ...
    if (D->MTDDriver->OTPAccess) {
	    result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_WRITE, (UINT)otp_addr, (void *)data_ptr, len);
    }
...
int32_t OTP_Read_Data(NOR_FLASH_DISK_Data *D, uint32_t otp_addr, uint8_t *data_ptr, uint32_t len)
{
    ...
    if (D->MTDDriver->OTPAccess) {
	    result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_READ, (UINT)otp_addr, (void *)data_ptr, len);
    }
...
int32_t OTP_Lock(NOR_FLASH_DISK_Data *D)
{
    ...
    if (D->MTDDriver->OTPAccess) {
	    result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_LOCK, NULL, NULL, 0);
    }
