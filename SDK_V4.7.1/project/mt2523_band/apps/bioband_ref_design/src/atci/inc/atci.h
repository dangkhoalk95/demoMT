 ...
 */
typedef enum {
    ATCI_STATUS_ERROR = 0,              /**< An error occurred during the function call. */
    ATCI_STATUS_OK = 1,                 /**< No error occurred during the function call. */
    ATCI_STATUS_REGISTRATION_FAILURE    /**< Failed to register the AT command handler table. */
} atci_status_t;
...
// This function initializes the ATCI module, about setting the spp server port id and init ATCI local paramters
// when spp server receive evet # BT_SPP_CONNECT_CNF.
typedef int8_t (* atci_send_data_by_spp_callback)(uint32_t len, uint8_t *data);

atci_status_t atci_init_by_spp(atci_send_data_by_spp_callback cb);
atci_status_t atci_deinit_by_spp(void);


// This function receive the AT cmd data from spp server, when spp server receive evet # BT_SPP_READY_TO_READ_IND.
