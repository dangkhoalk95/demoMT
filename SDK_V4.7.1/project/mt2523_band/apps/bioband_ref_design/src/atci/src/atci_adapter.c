...
void atci_queue_delete(uint32_t q_id)
{
	if(q_id != 0)
	{
		vQueueDelete((QueueHandle_t)q_id);
	}
...
void atci_mutex_delete(uint32_t mutex_id)
{
	if(mutex_id != 0)
	{
...
void atci_semaphore_delete(uint32_t semaphore_id)
{
	if(semaphore_id != 0)
	{
