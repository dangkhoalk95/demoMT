 ...
 * @{
 * @addtogroup mt7687_hdk_hal_examples_sha_get_hash sha_get_hash
 * @{
...
@par Directory contents
  ...
  - Project configuration files using Keil
    - \b MDK-ARM/sha_get_hash.uvprojx:
                             uVision5 project file. Contains the project
                             structure in XML format.
    - \b MDK-ARM/sha_get_hash.uvoptx:
  ...
  - Project configuration files using IAR
    - \b EWARM/sha_get_hash.ewd:  IAR project options. Contains the settings
                                  for the debugger.
    - \b EWARM/sha_get_hash.ewp:  IAR project file. Contains the project
                                  structure in XML format.
    - \b EWARM/sha_get_hash.eww:  IAR workspace file. Contains project
                                  information.
    - \b EWARM/flash.icf:         Linker script.

@par Run the example
  - Build the example project with the command, "./build.sh mt7687_hdk
    sha_get_hash" from the SDK root folder and download the bin file to
    LinkIt 7687 development board.
  - Connect your board to the PC with a micro USB cable.
  - Run the example. The result of its SHA calculation is displayed in the
    log.
*/
/**
 * @}
