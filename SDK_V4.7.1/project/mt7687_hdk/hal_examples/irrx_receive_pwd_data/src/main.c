...
#define IRRX_RC5_MODE_DEMO
//#define IRRX_PWD_MODE_DEMO


/* Private variables ---------------------------------------------------------*/

/* Private functions ---------------------------------------------------------*/
static void irrx_receive_rc5_data(void);
...
#ifdef IRRX_RC5_MODE_DEMO
static void irrx_rc5_callback(hal_irrx_event_t event, void  *user_data)
{
    hal_irrx_rc5_code_t data;

    if (HAL_IRRX_EVENT_TRANSACTION_SUCCESS == event)
    {
        hal_irrx_receive_rc5(&data);
        printf("receive rc5 data = 0x%08x\r\n",(unsigned int)data.code[0]);
        printf("receive rc5 data = 0x%08x\r\n",(unsigned int)data.code[1]);
    }
}

void irrx_receive_rc5_data(void)
{
    hal_gpio_init(HAL_GPIO_34);
    /* Call hal_pinmux_set_function to set GPIO pinmux, if EPT tool was not used to configure the related pinmux */
    hal_pinmux_set_function(HAL_GPIO_34, HAL_GPIO_34_IR_RX);
    ...
    hal_irrx_receive_rc5_start(HAL_IRRX_RC5_RECEIVE_FORMAT_SIGNAL_INVERSE | HAL_IRRX_RC5_RECEIVE_FORMAT_BIT_REVERSE , (hal_irrx_callback_t)irrx_rc5_callback, NULL);
    printf("wait for irtx to send RC5 data...\r\n");
    hal_gpio_deinit(HAL_GPIO_34);
}
#endif
...
#ifdef IRRX_RC5_MODE_DEMO
    irrx_receive_rc5_data();
#elif IRRX_PWD_MODE_DEMO
