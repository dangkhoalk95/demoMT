...
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
#define IRTX_RC5_MODE_DEMO
//#define IRTX_PWD_MODE_DEMO
...
/**
*@brief  Send RC5 mode IRTX data
*@param None.
*@return None.
*/
#ifdef IRTX_RC5_MODE_DEMO
static void irtx_send_RC5_data(void)
{
     uint8_t data[HAL_IRTX_MAX_DATA_LENGTH] = {0xAA, 0x55, 0xAA, 0x55,
                                               0x12, 0x34, 0x56, 0x78};
     printf("\r\n ---irtx_example begin---\r\n");
     hal_gpio_init(HAL_GPIO_33);
     hal_pinmux_set_function(HAL_GPIO_33, HAL_GPIO_33_IR_TX);//Set the GPIO33 to IRTX mode.
     hal_irtx_init();
     hal_irtx_send_rc5_data(HAL_IRTX_REPEAT_DISABLE,data, 56); //irrx max support 56 bits
     printf("\r\n ---irtx_send RC5 data done---\r\n");
     hal_gpio_deinit(HAL_GPIO_33);
}
#endif
