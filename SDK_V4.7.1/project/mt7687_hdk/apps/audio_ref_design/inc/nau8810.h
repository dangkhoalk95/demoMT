...
AUCODEC_STATUS_e aucodec_i2c_write(uint16_t u16Addr, uint16_t u16Data);

uint16_t aucodec_i2c_read(uint16_t u16Addr);
...
void aucodec_set_dai_fmt(AUCODEC_DAI_FORMAT_e eFormat, AUCODEC_DAI_WLEN_e eWLEN, AUCODEC_BCLK_INV_e eBCLKP);

void aucodec_set_mute(AUCODEC_MUTE_e eMuteEnable);

void aucodec_set_input(AUCODEC_INPUT_SEL_e eInputMode);

void aucodec_set_output(AUCODEC_OUTPUT_SEL_e eOutputMode);
