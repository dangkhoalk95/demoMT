...
int tls_selftest_main( int argc, char *argv[] )
{
    ...
    if( pointer != NULL )
    {
        LOG_E(mbedtls_proj, "all-bits-zero is not a NULL pointer\n" );
        return( -1 );
    }
    ...
    {
        LOG_E(mbedtls_proj, "The snprintf implementation is broken\n" );
        return( 0 );
    }
...
/* Slow tests last */
...
#else
    LOG_I(mbedtls_proj, " MBEDTLS_SELF_TEST not defined.\n" );
#endif
    ...
    if( v != 0 )
    {
        LOG_I(mbedtls_proj, "  [ All tests passed ]\n\n" );
#if defined(_WIN32)
