...
#if defined(CONFIG_MBEDTLS_HW_CRYPTO) && defined(MTK_MBEDTLS_HW_USE_NC_MEMORY)
ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN portSTACK_TYPE g_app_mbedtls_task_stack[APP_TASK_STACKSIZE / sizeof(portSTACK_TYPE)];
#endif
...
int main(void)
{
     ...
     */
#if defined(CONFIG_MBEDTLS_HW_CRYPTO) && defined(MTK_MBEDTLS_HW_USE_NC_MEMORY)
    xTaskGenericCreate(app_entry,
                       APP_TASK_NAME,
                       APP_TASK_STACKSIZE / sizeof(portSTACK_TYPE),
                       NULL,
                       APP_TASK_PRIO,
                       NULL,
                       g_app_mbedtls_task_stack,
                       NULL);
#else
