 ...
 * @{
 * @addtogroup mt7686_hdk_hal_examples_spi_slave_from_master spi_slave_from_master
 * @{

@par Overview
  - Example description
    - This example is a reference application to demonstrate how to use SPI
      slave API to communicate with an SPI master with two LinkIt 7686 HDKs.
    - This example does not require FreeRTOS.
  - Results
    - The result is in the log.
  - Features of the example project
    - The example project has two parts: 1. LinkIt 7686 HDK operates as SPI
      Slave device (spi_master_to_slave). 2. LinkIt 7686 HDK operates as SPI
      ...
      |                         |                     |                          |
      |_MT7686__________________|                     |_MT7686___________________|
      \endcode
...
@par Run the example
  - Build the example project for Master with a command "./build.sh
    mt7686_hdk spi_master_to_slave bl" from the SDK root folder and download
    the binary file to the LinkIt 7686 HDK that operates as SPI Master.
  - Build the example project for Slave with a command "./build.sh mt7686_hdk
    spi_slave_from_master bl" from the SDK root folder and download the
    binary file to the 7686 HDK that operates as SPI Slave.
  - Connect the related pins of the two LinkIt 7686 HDKs as shown in the
  ...
  - Run the example. The log will show the communication result with the SPI
    ...
    two LinkIt 7686 HDK boards (Master and Slave) are powered up at the same
