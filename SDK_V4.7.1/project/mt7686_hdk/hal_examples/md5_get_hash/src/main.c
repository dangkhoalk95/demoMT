...
uint8_t md5_digest[] = {
    0x40, 0x95, 0xd4, 0xa8, 0xfd, 0xb8, 0xd8, 0xb1, 0x7d, 0x58, 0xee, 0xaa, 0xf2, 0x5c, 0x9b, 0xaf
};
/* Private functions ---------------------------------------------------------*/
...
/**
*@brief Example of MD5 hash calculation.
*@param None.
*@return None.
*/
static void md5_get_hash_example(void)
{
    ...
    hal_md5_append(&context, data, size);
    hal_md5_end(&context, digest);

    printf("MD5 result:");
    md5_result_dump(digest, HAL_MD5_DIGEST_SIZE);
    if(memcmp(digest, md5_digest, HAL_MD5_DIGEST_SIZE)) {
