 ...
 * README for terms of use. The README is under the
 * <sdk_root>/middleware/third_party/libcoap folder.
 */
...
static unsigned char SER_DATA[150];

#define DATA_FROM_SERVER "This is a test server made with libcoap (see http://libcoap.sf.net)\n" \
   	      "Copyright (C) 2010--2013 Olaf Bergmann <bergmann@tzi.org>\n\n"
...
resolve_address(const str *server, struct sockaddr *dst) {
  ...
  if (error != 0) {
    ...
    LOG_I(coap_example, "getaddrinfo: error\n");
    return error;
  }
...
void
client_usage( const char *program, const char *version) {
  const char *p;
...
join( coap_context_t *ctx, char *group_name ){
  ...
  result = setsockopt( ctx->sockfd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
...
coap_context_t *
client_get_context(const char *node, const char *port) {
...
coap_client() {
...
#endif
  method = cmdline_method("get");
  coap_set_log_level(log_level);

  cmdline_uri( "coap://localhost" );
#if 0
  if ( optind < argc )
  ...
  case AF_INET:
    ...
    ctx = client_get_context("0.0.0.0", port_str);
    break;
  case AF_INET6:
    addrptr = &dst.addr.sin6.sin6_addr;

    /* create context for IPv6 */
    ctx = client_get_context("::", port_str);
    break;
  default:
    ;
  }
  ...
  if ((pdu = coap_new_request(ctx, method, optlist)) == NULL)
    return -1;
  ...
  if(strncmp((char *)SER_DATA, (char *)DATA_FROM_SERVER, strlen((char *)SER_DATA)) == 0) {
      LOG_I(coap_example, "example project test success.\n");
  } else {
      LOG_I(coap_example, "example project test fail.\n");
      LOG_I(coap_example, "SER_DATA: %s\n", SER_DATA);
      LOG_I(coap_example, "DATA_FROM_SERVER: %s\n", DATA_FROM_SERVER);
      LOG_I(coap_example, "example project test fail.\n");
  }
  close_output();
