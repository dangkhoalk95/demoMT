...
static bool bt_atci_parse_cmd(char *string)
{
    ...
    if (strstr(string, "AT+EBTTEST_DEMO=") != 0) {
        ...
        return result;
    } else if (strstr(string, "AT+EBTA2DP_DEMO=") != 0) {
        uint16_t codec = atoi(string + strlen("AT+EBTA2DP_DEMO="));
        LOG_I(common,"[BT_ATCI] A2DP codec control: codec:%d", codec);
        if (codec == 0) {
            bt_sink_srv_switch_aac_codec(false);
        } else {
            bt_sink_srv_switch_aac_codec(true);
        }
        return result;
    }  else if (strstr(string, "AT+EBTMP3_DEMO=1") != 0) {
        LOG_I(common, "enable");
        g_local_mp3_enable = 1;
    }  else if (strstr(string, "AT+EBTMP3_DEMO=0") != 0) {
        LOG_I(common, "disable");
        g_local_mp3_enable = 0;
    } 
    ...
    } else if (strstr(string, "AT+EBTMP3LP=1") != 0) {
        LOG_I(common, "MP3LP enable");
...
{"AT+EBTA2DP_DEMO", bt_atci_reg_callback, 0, 0},
