...
int32_t audio_file_manager_create_file_db(TCHAR * folder_name)
{
    ...
    while ((fres = f_readdir(&dir, &fno)) == FR_OK) {
        ...
        if (ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".MP3"), 4)
             &&(ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".mp3"), 4))
             &&(ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".WAV"), 4))
             &&(ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".wav"), 4))
             &&(ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".AAC"), 4))
             &&(ucs2_wcsncmp(fno.fname+file_name_len-4, _T(".aac"), 4))) {
            LOGI("skip other file!!");
            continue;
        }
