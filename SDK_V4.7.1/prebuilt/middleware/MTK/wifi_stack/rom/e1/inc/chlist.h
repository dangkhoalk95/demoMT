	...
	IN PUCHAR CntryCode)
{
	...
	while (rom_strcmp((char *)ChRegion[loop].CountReg, "") != 0)
	{
		if (rom_strncmp((char *)ChRegion[loop].CountReg, (char *)CntryCode, 2) == 0)
		{
...
#ifdef PHYMODE_TO_WDEV
	if (WMODE_CAP_N(pAd->CommonCfg.PhyMode)&& (pAd->CommonCfg.RegTransmitSetting.field.BW  == BW_40))
#else
...
static inline UCHAR N_SetCenCh(RTMP_ADAPTER *pAd, UCHAR prim_ch)
{
