...
*
* Configure APIs get and set the Wi-Fi driver configuration. In most cases they take effect
...
* it's restricted to call any in-band function similar to Wi-Fi configure or Wi-Fi connect APIs in wifi_rx_handler.
*
* The connect APIs manage the link status, such as: disconnect AP, disconnect the station, get the link status, get the station list, start or stop the scan.
* Register an event handler for scan, connect or disconnect event. The connect APIs use an in-band mechanism, thus these APIs must be called after the OS Task
...
typedef enum {
    ...
    WIFI_WPS_CONFIG_EXTERNAL_NFC_TOKEN      = 0x0010,  /**<  The NFC tag is used to do WPS.  */
...
*/
typedef struct {
    uint8_t opmode;                          /**< The operation mode. */
...
typedef struct {
    ...
    /**
         *@brief wep_key_length - WEP key length.
     */
...
/** @brief Specifies the Wi-Fi station items, including the Wi-Fi transmitter settings, the RSSI sample, MAC address, power save mode, bandwidth and keep alive status.
*/
typedef struct {
...
* @return  >=0 the operation completed successfully, <0 the operation failed.
*
* @note This function resets the Wi-Fi driver configuration to default values of the target's operation mode (including supplicant and firmware) previously set by Wi-Fi Configure APIs.
*/
...
/**
* @brief This function is only used for station mode and APCLI port of repeater mode.
*
* @param[out] bssid is the BSSID content, indicates the target AP's BSSID that device connected.
*                   if BSSID content is NULL, indicates the device is not connect yet.
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*/
...
/**
* @brief This function is only used for station mode and APCLI port of repeater mode. It must be called along with #wifi_config_set_ssid().
...
* receive those data.
*
* @param[out]  interval 1 ~ 255 beacon intervals.
...
* switches to idle state. After calling #wifi_config_reload_setting(), the device switches to scan state and try to connect to an AP router with an existing Wi-Fi
...
*
* @param[in] ssid is the specific SSID to scan. If the SSID is NULL, scan all APs based on channel tables. Scan with the SSID to find hidden APs.
...
/**
* @deprecated This is only for compatibility purpose, so that applications implemented with an earlier version of the API can work with the updated API. Please use #wifi_connection_register_event_handler() to replace this function.
*
* @param[in] event is the event ID. For more details, please refer to #wifi_event_t.
*
...
/**
* @deprecated This is only for compatibility purpose, so that applications implemented with an earlier version of the API can work with the updated API. Please use #wifi_connection_unregister_event_handler() to replace this function.
*
* @param[in] event is the event ID. For more details, please refer to #wifi_event_t.
*
* @param[in] notifier is the event handler. For more details, please refer to #wifi_event_handler_t.
