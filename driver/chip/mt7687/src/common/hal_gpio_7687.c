...
hal_gpio_pull_t halGPIO_Pull_get(UINT32 GPIO_pin)
{
    UINT16 no,flag;
    UINT16 remainder;
    uint32_t pull[2];
    ...
    remainder = GPIO_pin % 32;
    flag = 0;
    ret  = HAL_GPIO_PULL_ERROR;
    
    switch (no) {
        case 0:
            if (remainder <= 26) {
               flag = 1;
               pull[0] = (DRV_Reg32(IOT_GPIO_AON_BASE + IOT_GPIO_PU1)>>remainder)&0x1;
               pull[1] = (DRV_Reg32(IOT_GPIO_AON_BASE + IOT_GPIO_PD1)>>remainder)&0x1;  
            } 
            else if ((remainder>=27) && (remainder<=31)) {
                if (remainder == 27) {
        ...
        case 1:
            if (!remainder) {
                ...
                pull[0] = (DRV_Reg32(IOT_GPIO_AON_BASE + IOT_GPIO_PU2)>>remainder)&0x1;
                pull[1] = (DRV_Reg32(IOT_GPIO_AON_BASE + IOT_GPIO_PD2)>>remainder)&0x1;
                flag = 1;
            }
            break;
        default:
            return HAL_GPIO_PULL_ERROR;
    }

    if (flag == 1) {
       temp = (pull[0]<<4) + pull[1];  
       if (temp == 0x00) {
            ret = HAL_GPIO_PULL_DISABLE;
       }
       else if(temp == 0x01){
            ret = HAL_GPIO_PULL_DOWN;   
       }
       else if(temp == 0x10){
            ret = HAL_GPIO_PULL_UP; 
       }
       else {
            ret = HAL_GPIO_PULL_ERROR;
       }
        ...
        temp = (temp&0x81c);
        if ((temp== 0x804) || (temp== 0x808) || (temp== 0x80c)) {
            ret = HAL_GPIO_PULL_UP;
        }
        else if ((temp== 0x814) || (temp== 0x818) || (temp== 0x81c)){
            ret = HAL_GPIO_PULL_DOWN;
        }
        else if((temp== 0x800) || (temp== 0x810)){
            ret = HAL_GPIO_PULL_DISABLE;
        }
        else {
            ret = HAL_GPIO_PULL_ERROR;
    ...
    //printf("ret = %d, flag=%d, temp=%x  ",ret,flag,temp);
                        ...
                        UINT8 u1PrescalerCount)
{
    ...
    hal_nvic_register_isr_handler((IRQn_Type)irq_number, hal_eint_isr);
...
INT32 halRegGPInterrupt_set_triggermode(UINT32 pad_name,UINT8 edgeLevelTrig,UINT8 polarity,UINT8 dual)
{
   ...
   // printf("2-irq_number = %d, edgeLevelTrig = %d\r\n",irq_number, edgeLevelTrig);
   // printf("2-rdata=0x%x, reg_shift=%d\r\n", rdata, reg_shift);
   // printf("2-eint_cont[%d]=%x\r\n", int_index,eint_con_register->CM4_EINT_CON[int_index]);
   // printf("2-CM4_IRQ_SENS[%d]=%x\r\n\r\n", reg_index,eint_dbc_register->CM4_IRQ_SENS[reg_index]);
