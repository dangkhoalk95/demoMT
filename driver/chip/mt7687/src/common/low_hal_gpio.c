...
static gpio_backup_restore_t gpio_bakup_table[] = {
    /*reg_data, reg_addr*/
...
{
    return halRegGPInterrupt(gpio_name, edgeLevelTrig, debounceEnable , polarity , dual , prescaler, u1PrescalerCount);
}
...
int32_t eint_set_debounce(ENUM_IOT_GPIO_NAME_T gpio_name,
                 ...
                 uint8_t                    u1PrescalerCount)
{
    return halRegGPInterrupt_set_debounce(gpio_name,debounceEnable, prescaler, u1PrescalerCount);
}
...
hal_gpio_pull_t gpio_get_pull(ENUM_IOT_GPIO_NAME_T gpio_name)
{
    topPAD_Control_GPIO(gpio_name, ePAD_CONTROL_BY_CM4);
    return halGPIO_Pull_get(gpio_name);
}
