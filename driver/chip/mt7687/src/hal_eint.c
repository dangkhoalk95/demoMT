...
uint8_t hal_eint_table_index[IRQ_NUMBER_MAX];
...
void hal_eint_isr(hal_nvic_irq_t irq_number)
{
    hal_eint_callback_context_t *callback_context;

	uint32_t table_index;

	table_index = (uint32_t)hal_eint_table_index[irq_number];

	if (table_index>=hal_eint_count_max) {
      log_hal_info("[Eint]drvGPIO_IDX_EINT_WIC_LISR error, the irq_number=%d\r\n", irq_number);
      assert(0);
    }

	callback_context = &g_eint_callback_context[table_index];
        ...
        void *user_data)
{
	...
	table_index = get_index_from_gpio_pin(gpio_pin);
	
    hal_eint_callback_context_t *callback_context = &g_eint_callback_context[table_index];
    callback_context->callback = callback;
    callback_context->user_data = user_data;
    callback_context->callback_present = true;

	hal_eint_table_index[eint_gpio_table[table_index].irq_number] = table_index;
...
/*setting deounce 32K count, 1 count = 1/32K*/
hal_eint_status_t hal_eint_set_debounce_count(hal_eint_number_t eint_number, uint32_t count)
{
	...
	/*set clock source to 32K*/
    eint_set_debounce(gpio_pin, debounce_en, (ENUM_DEBOUNCE_PRESCALER_T)0, count);
    return HAL_EINT_STATUS_OK;
}
