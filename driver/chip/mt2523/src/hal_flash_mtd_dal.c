...
#ifdef __SECURITY_OTP__
    D->OTPLength = 1024; //0x200 - 0x40 = 448
    D->OTPBase = 0; //MXIC KGD 6 byte mark from 0x3A to 0x3F, thus OTP program region from 0x40
...
#endif
        {   
            NOR_FLASH_DISK_Data *Drv;
            savedMask = SaveAndSetIRQMask();
            Drv = &EntireDiskDriveData;
            D = Drv->MTDData;
            Drv->MTDDriver->MapWindow(Drv->MTDData, (BlockIndex & 0xFFFFFF), 0);
