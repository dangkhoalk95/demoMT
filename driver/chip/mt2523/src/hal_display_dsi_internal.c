...
#define DSI_POLLING_TIMEOUT 100000
...
hal_display_dsi_status_t display_dsi_set_command_queue(uint32_t *pData, uint8_t size, bool force_update)
{
    ...
    if (force_update) {
        ...
        while (lcd_dsi_register_ptr->LCD_DSI_INTSTA_REGISTER.field.CMD_DONE_INT_FLAG == 0) {
            ...
            if ((gpt_duration_count > DSI_POLLING_TIMEOUT) && (lcd_dsi_register_ptr->LCD_DSI_INTSTA_REGISTER.field.CMD_DONE_INT_FLAG == 0)) {
                return HAL_DISPLAY_DSI_STATUS_COMMAND_QUEUE_FAILED;
            }
        };
    }
...
hal_display_dsi_status_t display_dsi_start_bta_transfer(uint32_t cmd)
{
    ...
    while (lcd_dsi_register_ptr->LCD_DSI_INTSTA_REGISTER.field.TE_RDY_INT_FLAG == 0) {
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &gpt_current_count);
        hal_gpt_get_duration_count(gpt_start_count, gpt_current_count, &gpt_duration_count);
        if ((gpt_duration_count > DSI_POLLING_TIMEOUT) && (lcd_dsi_register_ptr->LCD_DSI_INTSTA_REGISTER.field.TE_RDY_INT_FLAG == 0)) {
            return HAL_DISPLAY_DSI_STATUS_COMMAND_QUEUE_FAILED;
        }
    };
