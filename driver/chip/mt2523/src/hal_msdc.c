...
volatile bool msdc_dvfs_registered = MSDC_FALSE;
...
void msdc_dma_wait_transfer_complete(msdc_port_t msdc_port,uint32_t address)
{
    ...
    while (1) {
        ...
        if (PDMA_OK != pdma_start_polling(dma_channel, dma_address)) {
            log_hal_error("DMA polling transfer data fail!\r\n");
            break;
        }
        ...
        if (0 != msdc_config[msdc_port].dma_count) {
...
void msdc_dma_isr(pdma_event_t msdc_event, void *user_data)
{
    ...
    if(PDMA_EVENT_TRANSACTION_SUCCESS != msdc_event) {
        log_hal_error("msdc_dma_isr event error\r\n");
    }

    dma_address = msdc_config[MSDC_PORT_0].dma_address;
    dma_count = msdc_config[MSDC_PORT_0].dma_count;
    is_write = msdc_config[MSDC_PORT_0].is_dma_write;

    if (0 != msdc_config[MSDC_PORT_0].dma_count) {
        msdc_dma_config(MSDC_PORT_0, dma_address, dma_count, is_write);
        msdc_set_dma_interrupt(MSDC_PORT_0, (uint32_t)dma_address);
        return;
    }
...
void msdc1_dma_isr(pdma_event_t msdc_event, void *user_data)
{
    ...
    if(PDMA_EVENT_TRANSACTION_SUCCESS != msdc_event) {
        log_hal_error("msdc1_dma_isr event error\r\n");
    }

    dma_address = msdc_config[MSDC_PORT_1].dma_address;
    dma_count = msdc_config[MSDC_PORT_1].dma_count;
    is_write = msdc_config[MSDC_PORT_1].is_dma_write;

    if (0 != msdc_config[MSDC_PORT_1].dma_count) {
        msdc_dma_config(MSDC_PORT_1, dma_address, dma_count, is_write);
        msdc_set_dma_interrupt(MSDC_PORT_1, (uint32_t)dma_address);
        return;
    }
...
msdc_status_t msdc_init(msdc_port_t msdc_port)
{
    ...
    if (msdc_dvfs_registered == MSDC_FALSE) {
        dvfs_register_notification(&msdc_desc);
        msdc_dvfs_registered = MSDC_TRUE;
    }
...
#endif
#ifdef HAL_DVFS_MODULE_ENABLED
    if ((msdc_config[MSDC_PORT_0].owner == MSDC_OWNER_NONE) && (msdc_config[MSDC_PORT_1].owner == MSDC_OWNER_NONE)) {
        irq_status = save_and_set_interrupt_mask();
        if (msdc_dvfs_registered == MSDC_TRUE) {
            dvfs_deregister_notification(&msdc_desc);
            msdc_dvfs_registered = MSDC_FALSE;
        }
        restore_interrupt_mask(irq_status);
    }
#endif
