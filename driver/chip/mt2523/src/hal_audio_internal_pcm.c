...
void audio_pcm_record_handle(bool enable)
{
    if (enable) {
        ...
        audio_pcm_record_on();
        if ((audio_pcm_is_single_mic() && (afe_get_path_type() != HAL_AUDIO_PLAYBACK_MUSIC_RECORD_VOICE))) {
            // because dsp memory is not enough to have booth play back and enhancement feature.
            // If the path type is HAL_AUDIO_PLAYBACK_MUSIC_RECORD_VOICE we will not enable enhancement.
