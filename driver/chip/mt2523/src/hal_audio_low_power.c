...
void audio_lowpower_sw_trigger_event_set(bool flag)
{
    if(flag){
#ifdef HAL_DVFS_MODULE_ENABLED
        sys_freq = hal_dvfs_get_cpu_frequency();   // query current system clock  config
        hal_dvfs_target_cpu_frequency(AUDIO_CPU_FREQ_L_BOUND, HAL_DVFS_FREQ_RELATION_H); // freq <=104M
#endif
        ...
        hal_dsp_topsm_get_resource(RM_MODEM_DSP_2, flag);
#ifdef HAL_DVFS_MODULE_ENABLED
        hal_dvfs_target_cpu_frequency(sys_freq, HAL_DVFS_FREQ_RELATION_H); // restore system clock config, freq <=sys_freq
#endif
    }
}
