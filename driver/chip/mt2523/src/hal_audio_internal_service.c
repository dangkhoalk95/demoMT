...
extern hal_audio_stream_t audio_config;
#ifdef HAL_DVFS_MODULE_ENABLED
static bool audio_dvfs_register_flag = false;
#endif

#if defined(HAL_SLEEP_MANAGER_ENABLED) && defined(HAL_AUDIO_LOW_POWER_ENABLED)
static bool audio_topsm_cb_flag = false;
#endif
...
#if defined(HAL_SLEEP_MANAGER_ENABLED) && defined(HAL_AUDIO_LOW_POWER_ENABLED)
void audio_enter_resume(void *data)
{
...
void audio_enter_suspend(void *data)
{
    if (audio_common.running_state != 0) {
...
#else // Don't affect IIR when we turn on/off prompt sound.
        uint16_t mask    = AUDIO_DSP_POST_PROCESSING_PCM_ROUTE_MASK;
        uint16_t control = AUDIO_DSP_POST_PROCESSING_PCM_ROUTE_OFF;
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
#ifdef HAL_AUDIO_LOW_POWER_ENABLED
        if ((audio_topsm_cb_flag == false) && (audio_lowpower_check_status())){
            hal_cm4_topsm_register_resume_cb((cm4_topsm_cb)audio_enter_resume, NULL);
            hal_cm4_topsm_register_suspend_cb((cm4_topsm_cb)audio_enter_suspend, NULL);
            audio_topsm_cb_flag = true;
        }
