...
void msdc_sdio_interrupt_handle(void)
{
#if defined(HAL_SDIO_MODULE_ENABLED)
    if (MSDC_OWNER_SDIO == msdc_get_owner()) {
        if (msdc_config.msdc_sdio_interrupt_callback) {
            msdc_config.msdc_sdio_interrupt_callback(HAL_SDIO_EVENT_SUCCESS,(void *)0);
        }else {
            log_hal_error("sdio interrupt callback is null!\r\n");
        }
    }
#endif
}
