...
#endif
#else
    LOG_I(inband, "cmd_id=%x, in_len=%d, out_len=%d, ext_cmd=%d", cmd_id, len, out_len, is_ext_cmd);
	err = inband_enqueue_cmd_config(cmd_id, buf, len, (uint8_t *)&status,
								 ret_buf, out_len, inband_default_rsp_handler,
								 TRUE, TRUE, TRUE, is_ext_cmd);
    LOG_HEXDUMP_I(inband, "inband_queue_query, ret_buf", ret_buf, out_len);
#endif
...
#endif
#else
    LOG_I(inband, "cmd_id=%x, in_len=%d, ext_cmd=%d", cmd_id, len, is_ext_cmd);

	err = inband_enqueue_cmd_config(cmd_id, buf, len, (uint8_t *)&status, NULL,
									0, inband_default_rsp_handler, TRUE, TRUE,
									FALSE, is_ext_cmd);
    LOG_HEXDUMP_I(inband, "inband_queue_exec, buf", buf, len);
#endif
    if (err < 0) {
        return err;
    }
	...
	P_CONNSYS_INBAND_REQ_T req, unsigned char *in_buffer)
{
    ...
    char * temp_buf = (char *)malloc(in_len + 1 + sizeof(INIT_HIF_TX_HEADER_T));
    if(temp_buf == NULL)
    {
        LOG_E(inband, "malloc tmp buf fail\n");
        return;
    }
    memset(temp_buf,0,in_len + 1 + sizeof(INIT_HIF_TX_HEADER_T));
...
int inband_send_cmd(inband_queue_tcb_t *iq_tcb, P_CONNSYS_INBAND_REQ_T req)
{
    unsigned char           cmd_id      = req->cmd_id;
    ...
    if(inband_tx_buf_lock(iq_tcb) != 0) {
        LOG_E(inband, "inband buf lock fail");
        return -1;
    }
    ...
    if (flow_control_status == CONNSYS_STATUS_SUCCESS) {
        ...
        LOG_W(inband, "flow control fail");
        inband_tx_buf_unlock(iq_tcb);
        return -1;
    }
...
int inband_queue_evt_handler(void *pkt_ptr, unsigned char *payload, unsigned int len)
{
    ...
    if (INBAND_HANDLE_NON_HANDLE == handled) {
        ...
        if (item) {
            if(p_rxd->ucEID == WIFI_EVENT_ID_IOT_SCAN_LIST && DlListLen(&iq_tcb->wait_list) != 0 && inband_get_mq_spaces() < 3) {
                //waiting inband response now, avoid fill full inband queue then cause net task been blocked.
                wifi_os_switch_context();
