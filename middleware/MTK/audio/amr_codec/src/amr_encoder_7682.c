...
static void amr_encoder_delete_memory_buffer(void)
{
    amr_encoder_internal_handle_t *internal_handle = amr_encoder_internal_handle;

    if (internal_handle->enc_int_buffer != NULL) {
        vPortFree(internal_handle->enc_int_buffer);
        internal_handle->enc_int_buffer = NULL;
    }

    if (internal_handle->enc_tmp_buffer != NULL) {
        vPortFree(internal_handle->enc_tmp_buffer);
        internal_handle->enc_tmp_buffer = NULL;
    }

    if (internal_handle->enc_pcm_buffer != NULL) {
        vPortFree(internal_handle->enc_pcm_buffer);
        internal_handle->enc_pcm_buffer = NULL;
    }

    if (internal_handle->enc_bs_buffer != NULL) {
        vPortFree(internal_handle->enc_bs_buffer);
        internal_handle->enc_bs_buffer = NULL;
    }

    if (internal_handle->encoded_pcm_buff.buffer_base_pointer  != NULL) {
        vPortFree(internal_handle->encoded_pcm_buff.buffer_base_pointer);
        internal_handle->encoded_pcm_buff.buffer_base_pointer  = NULL;
    }
...
amr_encoder_handle_t *amr_encoder_open(amr_encoder_callback_t amr_encoder_callback, const amr_encoder_config_t encoder_config)
{
    ...
    amr_encoder_handle_t *handle = NULL;
    amr_encoder_internal_handle_t *internal_handle = NULL;
    ...
    if (internal_handle->encoded_pcm_buff.buffer_base_pointer == NULL) {
        LOGE("Malloc encoded_pcm_buff failed\r\n");
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_int_buffer == NULL) {
        LOGE("Malloc enc_int_buffer failed\r\n");
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_tmp_buffer == NULL) {
        LOGE("Malloc enc_tmp_buffer failed\r\n");
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_pcm_buffer == NULL) {
        LOGE("Malloc enc_pcm_buffer failed\r\n");
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_bs_buffer == NULL) {
        LOGE("Malloc enc_bs_buffer failed\r\n");
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
