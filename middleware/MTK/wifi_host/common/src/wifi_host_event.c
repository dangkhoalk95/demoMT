...
void wifi_host_get_scan_result(uint8_t *payload)
{
    ...
    uint32_t index_temp = scan_list_temp->index;

    printf("scan index: %d, list size: %d.\r\n",(int)index_temp,(int)scan_ap_size_temp);
    ...
    for (i = 0; i < scan_ap_size_temp; i++) {
        os_memcpy(&g_scan_list[index_temp+i], &scan_ap_list_temp[i], sizeof(wifi_scan_list_item_t));
    }

    if((scan_ap_size_temp+index_temp) == g_scan_list_size) {
        wifi_rx_event_handler[WIFI_EVENT_IOT_SCAN_COMPLETE](WIFI_EVENT_IOT_SCAN_COMPLETE, NULL, 0);
    }
