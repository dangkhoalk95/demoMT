...
static bool sdio_send_pkt(unsigned char tx_que, uint8_t *tx_buf, int data_length)
{
    ...
    os_memset(&sdio_tx_buff, 0, BROM_BUFF_SIZE);
    buf = sdio_tx_buff;
    ...
    os_memcpy(buf, tx_buf , data_length);
...
static bool sdio_receive_pkt(uint8_t *rx_buf)
{
    ...
    os_memset(&whisr_enhance_reg , 0, sizeof(sdio_whisr_enhance_t));
...
bool xboot_over_sdio_fw_download(uint8_t *address, uint32_t length)
{
    ...
    // Send binary
    {
        ...
        for (; total_length > 0;) {
            os_memcpy(brom_tx_test, pBuf8, MAX_BLOCK_SIZE);
