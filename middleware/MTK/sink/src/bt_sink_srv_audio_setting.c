...
static void bt_sink_srv_audio_setting_update_callback(void *data)
{
    bt_sink_srv_report("[Sink][Setting]update_callback--data: 0x%x", data);
...
static void bt_sink_srv_audio_setting_gen_audio_vol_table(const audio_eaps_t *am_speech_eaps)
{
    ...
    for (i = 0; i < k; ++i) {
        ...
        bt_sink_srv_report("[Sink][Setting]gen_audio_vol(headset)--i: %d, gain: 0x%x, digital: 0x%x, analog: 0x%x", i, gain, digital, analog);
    }
    ...
    for (i = 0; i < k; ++i) {
        ...
        bt_sink_srv_report("[Sink][Setting]gen_audio_vol(handsfree)--i: %d, gain: 0x%x, digital: 0x%x, analog: 0x%x", i, gain, digital, analog);
    }
#else
    /* 2523 */
    k = BT_SINK_SRV_AUDIO_SETTING_STREAM_OUT_GAIN_LEV;
...
static void bt_sink_srv_audio_setting_gen_voice_vol_table(const audio_eaps_t *am_speech_eaps)
{
    ...
    for (i = 0; i < l; ++i) {
        ...
        bt_sink_srv_report("[Sink][Setting]gen_voice_vol(in)--i: %d, gain: 0x%x, digital: 0x%x, analog: 0x%x", i, gain, digital, analog);
    }
    ...
    for (i = 0; i < k; ++i) {
        ...
        bt_sink_srv_report("[Sink][Setting]gen_voice_vol(out)--i: %d, gain: 0x%x, digital: 0x%x, analog: 0x%x", i, gain, digital, analog);
    }
...
{
    bt_sink_srv_report("[Sink][Setting]init--ver: %d", PRODUCT_VERSION);
            ...
            mp3_vol_t *vol)
{
    ...
    if ((HAL_AUDIO_DEVICE_HEADSET == dev) || (HAL_AUDIO_DEVICE_HANDSET == dev)) {
        digital = audio_vol_out_table_headset[lev][0];
        analog = audio_vol_out_table_headset[lev][1];
    } else if (HAL_AUDIO_DEVICE_HANDS_FREE_STEREO == dev) {
        digital = audio_vol_out_table_handsfree[lev][0];
